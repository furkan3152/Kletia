...
describe("deterministic Arbitrum read-only intents", () => {
  it("keeps a safe borrow-capacity question read-only", async () => {
    const intent = await parseUserIntent(
      "Tell me how much USDC I can safely borrow on Aave V3 Arbitrum without sending a transaction",
      [],
      "arbitrum",
    );
  ...
  it("asks for the debt asset instead of inventing one", () => {
    expect(
      parseDeterministicArbitrumIntent(
        "Tell me my safe borrow capacity on Aave V3 Arbitrum",
      ),
    ).toMatchObject({ isComplete: false, action: "chat" });
  });
});
...
describe("wallet approval planning boundary", () => {
  it("prepares a positive swap route while keeping execution behind approval", async () => {
    const intent = await parseUserIntent(
      "Swap 10 USDC to WETH on Base Mainnet and show the exact quote before wallet approval",
      [],
      "base",
    );
  ...
  it("prepares a positive route when submission must wait for explicit approval", async () => {
    const intent = await parseUserIntent(
      "Swap 10 USDC to WETH on Base Mainnet; do not execute it until I explicitly approve",
      [],
      "base",
    );
  ...
  it("still rejects an actual transaction cancellation", async () => {
    const intent = await parseUserIntent(
      "Swap 10 USDC to WETH on Base Mainnet, but cancel that",
      [],
      "base",
    );
  ...
  it("grounds natural collateral placement language without inventing an action", async () => {
    const intent = await parseUserIntent(
      "I want to place 1 native USDC into the Kletia lending market on Arc Testnet as collateral; prepare a reviewable route before wallet approval",
      [],
      "arc",
    );
