...
export interface BaseSwapQuoteCollection {
    ...
    readonly quoteCollectionPolicyVersion:
        typeof BASE_SWAP_QUOTE_COLLECTION_POLICY;
...
function collectSourceRoutes(
    ...
    return result.value.flatMap((route) => {
        const checked = checkedQuoteCandidate(
            route,
            source,
            executionProfile,
        );
...
export async function collectBaseSwapQuotes(
    ...
    if (
        ...
        (
            normalizedTokenInSymbol === 'WETH' &&
            normalizedTokenOutSymbol === 'ETH'
        )
    ) {
        throw quoteCollectionError(
    ...
    let amountInWei = maxRequested
        ? 0n
        : parseUnits(requestedAmount, decimalsIn);
    ...
    const deadline = BigInt(
        Math.floor(Date.now() / 1_000) + 60 * 20,
    );
