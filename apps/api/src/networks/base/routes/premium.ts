...
} from "../intent/x402GatewayPolicy.js";
...
const X402_PAYMENT_ADDRESS = getAddress(
  configuredX402Treasury || "0xFf3a3CFC42D27E85DbA9Ea85f0bFEC34bd632f9A",
);
...
export function readOfficialCdpX402Credentials(
  ...
  const apiKeySecret = environment.CDP_API_KEY_SECRET?.trim().replace(
    /\\n/g,
    "\n",
  );
  ...
  const legacyApiKeySecret =
    environment.CDP_API_KEY_PRIVATE_KEY?.trim().replace(/\\n/g, "\n");
...
function resolvedPaymentMiddleware(
  ...
  const cacheKey = `${routeKey}|${payTo.toLowerCase()}|${priceAtomic}`;
  ...
  if (dynamicPaymentMiddlewareCache.size > MAX_DYNAMIC_PAYMENT_MIDDLEWARES) {
...
const dynamicX402Middleware = async (
  req: express.Request,
...
async function buildYieldSnapshot() {
  ...
  const topPools = payload.data
    ...
    .sort((left: any, right: any) => Number(right.apy) - Number(left.apy))
    .slice(0, 3)
    .map((pool: any) => ({
...
async function buildSybilSnapshot(address: Address) {
  ...
  if (firstTxTimestamp !== null && firstTxTimestamp < baseLaunchTimestamp) {
    firstTxTimestamp = baseLaunchTimestamp;
  }
  const walletAgeDays =
    firstTxTimestamp === null
      ? null
      : Math.max(0, Math.floor((Date.now() - firstTxTimestamp) / 86_400_000));
