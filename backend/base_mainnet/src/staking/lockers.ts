...
export async function getStakingRoutes(
    ...
    const safeSymbol = tokenInSymbol.toUpperCase() === 'ETH'
        ? 'WETH'
    ...
    if (safeSymbol === 'WELL') {
        routes.push({
            ...
            amount,
            expectedOutput:
                `Stake ${formattedAmount} WELL for stkWELL; live rewards are not projected`,
    ...
    if (safeSymbol === 'SEAM') {
        routes.push({
            ...
            amount,
            expectedOutput:
                `Deposit ${formattedAmount} SEAM for stkSEAM; live rewards are not projected`,
