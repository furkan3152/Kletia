...
function routeAfterSimulation<T extends object>(
  ...
  return {
    ...route,
    simulationStatus: simulation.success ? "passed" : "deferred_until_approval",
  };
}
...
export async function handleSmartSwap(
  ...
  if (!intent.tokenIn)
    throw new Error("🚨 Target token could not be determined.");
  assertBaseProtocolConstraintCompatibility(
    intent.protocol,
  ...
  if (
    (tIn.toUpperCase() === "ETH" && tOut.toUpperCase() === "WETH") ||
    (tIn.toUpperCase() === "WETH" && tOut.toUpperCase() === "ETH")
  ) {
    ...
    if (maxRequested) {
      ...
      amountInWei = isWrap ? (bal > gasReserve ? bal - gasReserve : 0n) : bal;
    }
    if (amountInWei <= 0n)
      throw new Error(
        `AMOUNT_REQUIRED: Amount must be positive or explicitly MAX.`,
      );
    if (bal < amountInWei)
      throw new Error(
        `KEE_ERROR|INSUFFICIENT_FUNDS|Insufficient Balance|Insufficient balance. Direct the user to fund their wallet. [SHOW_ONRAMP]`,
      );
    ...
    const routeObj = {
      name: isWrap ? "WETH Contract (Wrap)" : "WETH Contract (Unwrap)",
      amountOut: amountInWei,
      expectedOutput: isWrap
        ? `Wrap ${formatUnits(amountInWei, 18)} ETH to WETH`
        : `Unwrap ${formatUnits(amountInWei, 18)} WETH to ETH`,
      routePath: isWrap ? `ETH ➝ WETH` : `WETH ➝ ETH`,
      router: wethAddr,
    ...
    return {
      status: "success",
      winner: verifiedRoute.name,
      expectedOutput: verifiedRoute.expectedOutput,
      routePath: verifiedRoute.routePath,
      targetContract: wethAddr,
      calldata: verifiedRoute.calldata,
      tokenInAddress: undefined,
      amountInWei: amountInWei.toString(),
      isNativeIn: isWrap,
      value: verifiedRoute.value,
      allRoutes: [verifiedRoute],
    };
  }
  ...
  if (intent.protocol && intent.protocol !== "unknown") {
    ...
    if (finalRoutes.length === 0)
      throw new Error(
        `🚨 Only the "${intent.protocol}" protocol was requested, but no secure route was found for this transaction.`,
      );
  }
  ...
  return {
    ...
    quoteCoverage,
    rankingEvidence,
    protocolExclusionEvidence: quoteCollection.protocolExclusionEvidence,
    winnerMessage:
...
export async function handleLiquidity(
  ...
  if (intent.tokenOut) {
    await checkTokenSecurity(requiredTokenAddress(intent.tokenOut, "tokenOut"));
  }
  ...
  if (rawAction === "addliquidity" || rawAction === "add_liquidity") {
    ...
    rawAction === "removeliquidity" ||
    rawAction === "remove_liquidity"
  ) {
  ...
  const protocolExclusionResult = applyBaseProtocolExclusions<LiquidityRoute>(
  ...
  const removalComparisonUnavailable = safeAction === "remove_liquidity";
  const liquidityRoutingEvidence = {
    ...
    direction: removalComparisonUnavailable ? "not_applicable" : "descending",
    ...
    limitation: removalComparisonUnavailable
  ...
  return {
    ...
    calldata: w.calldata,
    tokenInAddress: w.primaryTokenAddress || w.secondaryTokenAddress,
    amountInWei: w.primaryAmountInWei || w.secondaryAmountInWei || "0",
...
export async function handleDeFiBanking(intent: ParsedIntent, user: string) {
  ...
  if (verified.length === 0) {
    if (lastError) {
      const analyzed = KletiaErrorTracker.analyzeError(
        lastError,
        intent.action,
      );
      throw new Error(
  ...
  const riskTolerance =
    ...
    intent.riskTolerance === "aggressive"
      ...
      : "balanced";
  const { rankedRoutes: finalRoutes, yieldRankingEvidence } =
  ...
  return {
    ...
    targetContract: winner.router,
    calldata: winner.calldata,
    tokenInAddress: needsAllowance ? winner.primaryTokenAddress : undefined,
...
export async function handleYieldCompare(intent: ParsedIntent) {
  ...
  const result = await getLendingOpportunities(
    ...
    intent.protocol,
    riskTolerance,
    intent.objective === "lowest_borrow_cost" ? "borrow" : "supply",
  );
  const protocolExclusionResult = applyBaseProtocolExclusions(
      ...
      opportunities
        .map((opportunity) => {
          ...
          return `${opportunity.name} ${
            rate === null ? "rate unavailable" : `${rate / 100}%`
...
export async function handleStaking(intent: ParsedIntent, user: string) {
  ...
  const quotedRoutes = await getStakingRoutes(
    safeToken,
    intent.amount!,
    intent.durationInDays || 30,
    user,
    intent.protocol,
  );
  const protocolExclusionResult = applyBaseProtocolExclusions(
  ...
  for (const route of rawRoutes) {
    const tokensToScan = [
      {
        addr: isNative ? undefined : TOKENS[safeToken],
        amt: route.amount?.toString(),
      },
    ];
    const sim = await xRaySimulate(
      route.router as `0x${string}`,
...
export async function handleLiquidStaking(intent: ParsedIntent, user: string) {
  const action = intent.action.toLowerCase().includes("unstake")
    ? "liquid_unstake"
    : "liquid_stake";
  ...
  const candidates = [intent.tokenIn, intent.tokenOut, intent.protocol]
...
export async function handleBridge(intent: ParsedIntent, user: string) {
  ...
  if (!intent.destinationChain)
    throw new Error(
      "🚨 Please specify the destination network (e.g., arbitrum, optimism).",
    );
  const excludedProtocolIds = assertBaseProtocolConstraintCompatibility(
    intent.protocol,
    intent.excludedProtocols,
  ...
  if (maxRequested) {
    const gasReserve = parseUnits("0.001", 18);
    amountInWei = isNative ? (bal > gasReserve ? bal - gasReserve : 0n) : bal;
  }
  if (amountInWei <= 0n || bal < amountInWei)
    throw new Error(
      `KEE_ERROR|INSUFFICIENT_FUNDS|Insufficient Balance|Insufficient balance. Not enough tokens in wallet. Direct the user to fund their wallet. [SHOW_ONRAMP]`,
    );

  const quotedRoutes = await getAcrossBridgeRoutes(
    tAddr,
    tIn,
    amountInWei,
  ...
  for (const route of rawRoutes) {
    const tokensToScan = [
      { addr: isNative ? undefined : tAddr, amt: amountInWei.toString() },
    ];
    const sim = await xRaySimulate(
      route.router as `0x${string}`,
      route.calldata as `0x${string}`,
