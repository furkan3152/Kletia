...
import {
    executeBaseIntentV2Swap,
} from './baseIntentRouterV2Integration.js';
import {
    agentLogRoom,
    emitAgentLog,
} from '../observability/agentLog.js';
...
function assertExplicitPositiveAmount(action: string, amount: unknown) {
    ...
    if (!isMax && !isPositiveDecimal) {
        throw Object.assign(
            new Error('İşlem için pozitif bir miktar veya açıkça MAX belirtilmelidir.'),
...
export async function handleAlloraPrediction(asset: string, userAddress: string) {
    ...
    if (!ALLORA_ASSETS.includes(normalizedAsset as (typeof ALLORA_ASSETS)[number])) {
        throw Object.assign(
    ...
    if (!apiKey) {
        throw Object.assign(
            new Error('Live prediction service is unavailable.'),
            { code: 'ALLORA_UNAVAILABLE', statusCode: 503 },
        );
    }
...
export async function executeKletiaEngine(
    ...
    try {
        ...
        const tokenInSymbol =
            workingIntent.tokenIn?.trim().toUpperCase();
        const tokenOutSymbol =
            workingIntent.tokenOut?.trim().toUpperCase();
        ...
        switch (action) {
            ...
            case 'borrow':
            case 'repay':
            case 'withdraw': result = await handleDeFiBanking(workingIntent, userAddress); break;
            case 'stake': result = await handleStaking(workingIntent, userAddress); break;
            case 'liquid_stake':
            case 'liquid_unstake': result = await handleLiquidStaking(workingIntent, userAddress); break;
            case 'add_liquidity':
            case 'remove_liquidity': result = await handleLiquidity(workingIntent, userAddress); break;
            case 'bridge': result = await handleBridge(workingIntent, userAddress); break;
                ...
                if (!sim.success) {
                    ...
                    const simulationDetail =
                        typeof simulationError?.shortMessage === 'string'
                            ...
                            : 'Reverted';
                    throw new Error(`Ağ Kuralları İhlali: Bu işlem ağ tarafından reddediliyor. Detay: ${simulationDetail}`);
                }
                break;
            case 'deploy_token':
                ...
                if (!deploymentSimulation.success) {
                    throw Object.assign(
                        new Error('Token deployment transaction failed live Base simulation.'),
                        {
        ...
        if (feeData && result.status === 'success' && !(result.winner && result.winner.includes('WETH Contract'))) {
            ...
            result.feeRouterCoverage = {
                ...
                unapprovedTargetCount: targetChecks.filter(
                    ([, approved]) => !approved,
                ).length,
            ...
            if (wrappedRoutes.length === 0) {
                throw Object.assign(
                    new Error('No route passed the Kletia fee-router allowlist and simulation.'),
            ...
            result.tokenInAddress = isNative
                ? undefined
