...
export async function executeKletiaEngine(intent: ParsedIntent, userAddress: string, originalPrompt: string = "", msgId: string = "", isArcMode: boolean = false) {
    try {
        ...
        // ✨ ARC NETWORK İŞLEMLERİ — Backend üzerinden Native Encoding
        const arcActions = ['arc_swap', 'arc_stake', 'arc_unstake', 'arc_vault_deposit', 'arc_vault_withdraw', 'arc_memo_send', 'arc_batch_pay', 'arc_add_liquidity', 'arc_remove_liquidity', 'arc_post_job', 'arc_register_agent', 'arc_lending_deposit', 'arc_lending_borrow', 'arc_lending_repay'];
        if (isArcMode || arcActions.includes(intent.action)) {
