...
contract KletiaArcAgentRegistry is ERC2771Context {
    ...
    modifier onlyOwner() {
        require(
            _msgSender() == owner,
            "KletiaArcAgentRegistry: caller is not the owner"
        );
        _;
    }
    ...
    modifier onlyAgentOwner(uint256 agentId) {
        require(
            agentId < nextAgentId,
            "KletiaArcAgentRegistry: agent does not exist"
        );
        require(
    ...
    function registerAgent(
        ...
        require(
            bytes(name).length > 0,
            "KletiaArcAgentRegistry: name is required"
        );
        require(
            bytes(name).length <= 128,
            "KletiaArcAgentRegistry: name too long"
        );
        require(
            bytes(description).length > 0,
            "KletiaArcAgentRegistry: description is required"
        );
        require(
            bytes(description).length <= 1024,
            "KletiaArcAgentRegistry: description too long"
        );
        require(
            skills.length > 0,
            "KletiaArcAgentRegistry: at least one skill required"
        );
        require(skills.length <= 20, "KletiaArcAgentRegistry: too many skills");
        require(
            bytes(endpointUrl).length > 0,
            "KletiaArcAgentRegistry: endpoint URL is required"
        );
        require(
            bytes(endpointUrl).length <= 256,
            "KletiaArcAgentRegistry: endpoint URL too long"
        );
        ...
        for (uint256 i = 0; i < skills.length;) {
            require(
                bytes(skills[i]).length > 0,
                "KletiaArcAgentRegistry: empty skill tag"
            );
            require(
                bytes(skills[i]).length <= 64,
                "KletiaArcAgentRegistry: skill tag too long"
            );
    ...
    function updateAgent(
        ...
        require(
            bytes(name).length > 0 && bytes(name).length <= 128,
            "KletiaArcAgentRegistry: invalid name"
        );
        require(
            bytes(description).length > 0 && bytes(description).length <= 1024,
            "KletiaArcAgentRegistry: invalid description"
        );
        require(
            bytes(endpointUrl).length > 0 && bytes(endpointUrl).length <= 256,
            "KletiaArcAgentRegistry: invalid endpoint"
        );
    ...
    function deactivateAgent(uint256 agentId) external onlyAgentOwner(agentId) {
        require(
            _agents[agentId].active,
            "KletiaArcAgentRegistry: already inactive"
        );
    ...
    function reactivateAgent(uint256 agentId) external onlyAgentOwner(agentId) {
        require(
            !_agents[agentId].active,
            "KletiaArcAgentRegistry: already active"
        );
    ...
    function updateReputation(
        ...
        require(
            newScore <= 10_000,
            "KletiaArcAgentRegistry: score out of range (0-10000)"
        );
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(
            newOwner != address(0),
            "KletiaArcAgentRegistry: invalid new owner"
        );
