const LOCAL_BACKEND_ORIGIN = 'http://127.0.0.1:3001';
...
const allowLocalBackend =
  !isProductionBuild ||
  import.meta.env.VITE_ALLOW_LOCAL_BACKEND === 'true';
const candidate = configuredBackendOrigin ||
  (allowLocalBackend ? LOCAL_BACKEND_ORIGIN : '');
...
if (!allowLocalBackend && (localHostname || parsedBackendUrl.protocol !== 'https:')) {
  throw new Error(
