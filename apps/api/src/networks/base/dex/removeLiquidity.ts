...
async function buildPoolRemoval(
  ...
  const amountAMin = minimumAfterSlippage(expectedA, slippageBps);
  const amountBMin = minimumAfterSlippage(expectedB, slippageBps);
  ...
  return {
    ...
    routePath:
      `[${pool.protocolName}] LP ➝ ` + `${tokenASymbol} + ${tokenBSymbol}`,
    router: pool.router,
    calldata,
    ...
    poolEvidence: {
      ...
      limitation: `Outputs are pro-rata reserve estimates with ${slippageBps} bps minimums; transfer fees, reserve changes and execution gas can change realized amounts.`,
    },
  };
}
