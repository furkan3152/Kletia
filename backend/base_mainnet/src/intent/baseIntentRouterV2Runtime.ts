...
} from './baseIntentRouterV2RuntimeAbis.js';
...
type RuntimeRead = (
    parameters: RuntimeReadParameters,
) => Promise<unknown>;
...
function checkedRuntimeCode(value: unknown): Hex {
    if (
        typeof value !== 'string' ||
        !RUNTIME_CODE_PATTERN.test(value)
    ) {
        configError();
    }
    return value.toLowerCase() as Hex;
...
function normalizeInput(
    ...
    if ('mode' in record) {
        ...
        if (
            !record.deployment ||
            typeof record.deployment !== 'object'
        ) {
            configError();
        }
        deployment =
            record.deployment as BaseIntentV2DeploymentEvidence;
    } else {
        configuredRouter = checkedAddress(record.router);
        deployment =
            input as BaseIntentV2DeploymentEvidence;
    }
...
async function validateRouterState(
    ...
    deployment.adapters.forEach((adapter, index) => {
        const [
            ...
            configHash,
        ] = checkedAdapterConfigTuple(
            adapterConfigResults[index],
        );
        if (
            !configured ||
            !enabled ||
...
async function validateAdapterState(
    ...
    const [
        ...
        isUniV3
            ? readAdapter('ADAPTER_FORMAT_VERSION')
            : Promise.resolve(null),
    ]);

    if (
        !sameHex(
            checkedBytes32(actionKindResult),
            KLETIA_SWAP_ACTION_KIND_V2,
        ) ||
        !sameAddress(checkedAddress(targetResult), adapter.target) ||
        !sameAddress(
            checkedAddress(spenderResult),
            adapter.spender,
        ) ||
        !sameAddress(
            checkedAddress(factoryResult),
            adapter.factory,
        ) ||
        !sameAddress(
            checkedAddress(wrappedNativeResult),
        ...
        !sameHex(
            checkedBytes32(targetCodehashResult),
            adapter.targetCodehash,
        ) ||
        !sameHex(
            checkedBytes32(factoryCodehashResult),
        ...
        !sameAddress(
            checkedAddress(routerFactoryResult),
            adapter.factory,
        ) ||
        !sameAddress(
...
async function validateRuntimeUnsafe(
    ...
    const blockNumber = checkedBlockNumber(
        await client.getBlockNumber(),
    );
    if (deployment.observedAtBlock > blockNumber) configError();

    const read = client.readContract.bind(
        client,
    ) as unknown as RuntimeRead;
