...
export const ArcLendingDashboard: React.FC<LendingDashboardProps> = ({ onActionClick }) => {
  ...
  const colUsdValue =
    collateral === null || kletPrice === null
      ? null
      : collateral * kletPrice;
    ...
    <div className="w-full max-w-7xl mx-auto space-y-8 p-4 md:p-8 animate-fade-in pb-20">
              ...
              <span className="text-xl font-black text-[#1A1A1A] flex items-center gap-2">
                {isPriceReadPending
                  ...
                  : kletPrice === null
                    ? 'Unavailable'
                    : `$${kletPrice.toFixed(8)}`}{' '}
                <span className="text-xs text-[#10B981] bg-[#D1FAE5] px-2 py-0.5 border-[2px] border-[#10B981]">USDC</span>
              </span>
            </div>
            <div className="bg-white border-[3px] border-[#1A1A1A] p-4 shadow-[4px_4px_0_#1A1A1A] min-w-[140px]">
              <div className="flex flex-col">
                <span className="text-xs font-black text-gray-500 uppercase block mb-1">Protocol Status</span>
                <span className={`text-2xl font-black flex items-center gap-2 ${protocolStatus === 'AT RISK' || protocolStatus === 'UNAVAILABLE' ? 'text-[#EF4444]' : 'text-[#10B981]'}`}>
                  <span className={`w-3 h-3 border-[2px] border-[#1A1A1A] rounded-full ${protocolStatus === 'AT RISK' || protocolStatus === 'UNAVAILABLE' ? 'bg-[#EF4444]' : 'bg-[#10B981] animate-pulse'}`}></span>
                  {protocolStatus}
                </span>
              </div>
            </div>
        ...
        <div className="z-10 bg-white dark:bg-[#0F172A] border-[4px] border-[#1A1A1A] shadow-[6px_6px_0_#1A1A1A] p-6 lg:min-w-[300px] flex flex-col justify-center">
          <div className="mb-4">
            ...
            <div className="text-xl font-bold text-gray-600 dark:text-gray-400 mt-2 flex items-center gap-2">
              {isArcConnected && !isKletBalanceError && kletRawBalance !== undefined
                ? Number(ethers.formatEther(kletRawBalance as bigint)).toFixed(6)
                : '—'}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <div className="flex justify-between items-end mb-2">
              <span className="font-bold text-gray-600 dark:text-gray-400">
                LTV Usage {ltvPercent === null ? '(loading on-chain limit)' : `(Max ${ltvPercent.toFixed(0)}%)`}
              </span>
            ...
            {!isHealthy && healthFactor !== null && borrowed !== null && borrowed > 0 && (
        ...
        <div className="bg-white dark:bg-[#1E293B] border-[4px] border-[#1A1A1A] dark:border-[#4B5563] p-6 shadow-[6px_6px_0_#1A1A1A] dark:shadow-[6px_6px_0_#475569]">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-gray-500 mt-1">
              {colUsdValue === null ? 'Live value unavailable' : `≈ $${colUsdValue.toFixed(2)}`}
            </div>
            <button 
        ...
        <div className="bg-white dark:bg-[#1E293B] border-[4px] border-[#1A1A1A] dark:border-[#4B5563] p-6 shadow-[6px_6px_0_#1A1A1A] dark:shadow-[6px_6px_0_#475569]">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-gray-500 mt-1">
              Max Borrowable: {maxBorrowUsd === null ? 'Unavailable' : `$${maxBorrowUsd.toFixed(2)}`}
            </div>
            <button 
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { icon: '💰', title: 'Add Collateral', prompt: 'Add KLET collateral to Kletia lending protocol' },
            { icon: '💸', title: 'Borrow', prompt: 'Borrow USDC via Kletia lending' },
            { icon: '💳', title: 'Repay', prompt: 'Repay my USDC debt on Kletia lending' },
            { icon: '🔓', title: 'Withdraw Collateral', prompt: 'Withdraw KLET collateral via Kletia lending' }
          ].map((action, idx) => (
            <button
              key={idx}
