...
function protocolConstraintError(
    code: string,
    message: string,
): Error {
...
export function isBaseProtocolExcluded(
    ...
    return excludedProtocolIds.some((excluded) => {
        ...
        return PROTOCOL_EXCLUSION_FAMILIES[excluded]?.has(
            normalizedProtocol,
        ) === true;
    });
}
...
export function assertBaseProtocolConstraintCompatibility(
    ...
    const excludedProtocolIds =
        normalizeExcludedBaseProtocols(excludedProtocols);
    const normalizedRequested =
        normalizeBaseProtocolId(requestedProtocol);
    if (
        normalizedRequested &&
...
export function applyBaseProtocolExclusions<
    T extends ProtocolIdentifiedRoute,
>(
    routes: readonly T[],
    excludedProtocolIds: readonly string[],
): {
