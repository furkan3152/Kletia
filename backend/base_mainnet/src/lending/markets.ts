...
interface LendingMarketSnapshot {
    ...
    readonly actionEnabled: Readonly<
        Record<BaseLendingAction, boolean>
    >;
...
async function readErc4626Snapshot(
    ...
    return {
        ...
        actionEnabled: {
            ...
            repay: false,
            withdraw:
                maxWithdrawAtomic === null || maxWithdrawAtomic > 0n,
        },
        observedAt: new Date().toISOString(),
    };
}
...
async function collectMarketSnapshots(
    ...
    const safeSymbol = tokenSymbol.toUpperCase() === 'ETH'
        ? 'WETH'
    ...
    const snapshots = settled
        .filter(
            (
                result,
            ): result is PromiseFulfilledResult<LendingMarketSnapshot> =>
        ...
        .filter(
            (snapshot) =>
                snapshot.active &&
                protocolMatches(snapshot, requestedProtocol),
        );
...
function routeEconomics(
    ...
    return {
        ...
        estimateStatus: rateBps === null && (action === 'lend' || action === 'borrow')
            ? 'partial'
            : 'complete',
...
function formattedRate(rateBps: number | null): string {
    return rateBps === null ? 'rate unavailable' : `${(rateBps / 100).toFixed(2)}% annualized rate snapshot`;
}
...
function buildLendingRoute(
    ...
    if (snapshot.protocolId === 'aave-v3') {
        ...
        if (action !== 'lend' && action !== 'withdraw') {
            throw new Error(
                `${snapshot.protocolId} does not support ${action}.`,
            );
        }
        calldata =
            action === 'lend'
...
export async function getLendingOpportunities(
    ...
    const definition =
        BASE_TOKEN_REGISTRY[
            safeSymbol as keyof typeof BASE_TOKEN_REGISTRY
        ];
    if (!definition) {
        throw Object.assign(
    ...
    const ranked = (riskFiltered.length > 0 ? riskFiltered : opportunities)
        .sort((left, right) => {
            const leftRate =
                comparison === 'supply'
                    ? left.supplyRateBps
            ...
            const riskOrder =
                riskRank[left.riskTier] - riskRank[right.riskTier];
    ...
    return {
        ...
        coverage: {
            registeredProtocolCount:
                Number(Boolean(AAVE_V3_BASE.reserves.find(({ token }) => token === safeSymbol))) +
                Number(Boolean(MOONWELL_BASE.markets.find(({ token }) => token === safeSymbol))) +
                COMPOUND_V3_BASE.markets.filter(({ token }) => token === safeSymbol).length +
