...
contract KletiaArcVault is ERC2771Context {
    ...
    modifier onlyOwner() {
        require(
            _msgSender() == owner,
            "KletiaArcVault: caller is not the owner"
        );
        _;
    }
    ...
    function deposit() external payable nonReentrant {
        ...
        if (d.principal > 0) {
            d.accruedInterest += _calculateInterest(
                d.principal,
                d.lastAccrualTimestamp
            );
        }
    ...
    function withdraw() external nonReentrant {
        ...
        uint256 interest =
            d.accruedInterest +
                _calculateInterest(d.principal, d.lastAccrualTimestamp);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        uint256 forfeited =
            d.accruedInterest +
                _calculateInterest(principal, d.lastAccrualTimestamp);
