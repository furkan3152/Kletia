...
} from '../config/baseLaunchFactoryV2';
import {
  BACKEND_URL,
  BASE_PAYMASTER_ENABLED,
} from '../config/runtime';
...
export function resolveBaseWalletExecutionCapabilities(
  ...
  const isBaseMainnet =
    network === 'base' && chainId === BASE_MAINNET_CHAIN_ID;
...
export function assertIntentV2TransactionPlanBoundary(
  ...
  const isIntentRouterV2 =
    plan.executionMode === 'kletia_intent_router_v2';
  if (!isIntentRouterV2) {
    if (
  ...
  if (
    ...
    suppliedTargets.some(
      (target, index) =>
        getAddress(target) !== expectedTargets[index],
    ) ||
    new Set(
      suppliedTargets.map((target) =>
...
export function assertLaunchFactoryV2TransactionPlanBoundary(
  ...
  const isLaunchFactoryV2 =
    plan.executionMode === 'kletia_launch_factory_v2';
...
export function assertTransactionReturnData(
  ...
  const [returnCode] = decodeAbiParameters(
    [{ type: 'uint256' }],
    data,
  );
  if (returnCode !== 0n) {
  ...
  const scanAddress = useCallback(async (
    ...
    try {
      ...
      requestUrl.searchParams.set(
        'chainId',
        String(expectedNetwork.chainId),
      );
      if (normalizedAction) {
      ...
      if (
        ...
        throw new Error(`Güvenlik kontrolü bu adresi yüksek riskli olarak işaretledi${tags}.`);
      }
      if (
        network === 'arc' &&
        (
  ...
  const execute = useCallback(
    async (
      ...
      if ((!bytecode || bytecode === '0x') && plan.data !== '0x') {
        throw new Error('Calldata içeren işlem hedefi geçerli bir akıllı sözleşme değil.');
      }
      if (
      ...
      if (policyTargets.length > 0) {
        onLog(
          `✅ ${policyTargets.length} iç politika hedefi ayrıca tarandı.`,
        );
      }
      ...
      for (const approval of approvalRequirements.values()) {
        ...
        onLog(`🔬 ${approval.symbol || 'Token'} approval simülasyonu çalıştırılıyor.`);
      ...
      const atomicCalls = buildAtomicCallBatch(
        missingApprovals,
        actionCall,
      );
      const walletExecutionCapabilities = resolveBaseWalletExecutionCapabilities(
        plan.network,
        plan.chainId,
        capabilities,
      );
      const canUseAtomicCalls =
        walletExecutionCapabilities.canUseAtomicCalls;
      const canUsePaymaster =
        BASE_PAYMASTER_ENABLED &&
        walletExecutionCapabilities.canUsePaymaster;
