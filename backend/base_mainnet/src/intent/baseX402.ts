...
function configuredServerCap(): bigint {
  const raw =
    process.env.X402_BUYER_MAX_PAYMENT_USDC?.trim() ||
    DEFAULT_SERVER_CAP;
...
export async function discoverBaseX402Services(input: {
  ...
  const services = payload.resources
      .map((resource) =>
        validateBaseX402Resource(resource, cap.atomic),
      )
      .filter((service): service is BaseX402Service => service !== null)
      .filter(
        (service) =>
          !effectiveCuratedOnly || service.curated === true,
      )
      .slice(0, MAX_RESULTS);
...
export async function preflightExplicitBaseX402GetPrompt(
  ...
  let walletInputBinding:
    | BaseX402ChallengeEvidence['walletInputBinding']
    | undefined;
  if (missingParams.length > 0) {
    const parameter = missingParams.length === 1
      ? missingParams[0].toLowerCase()
      : '';
    ...
    if (
      (parameter !== 'address' && parameter !== 'wallet') ||
      !walletAddress
    ) {
      throw new BaseX402IntentError(
        'X402_REQUIRED_INPUT_MISSING',
...
export function buildBaseMcpX402Plan(
  ...
  let url = sourceUrl;
  const method = String(intent.method || 'GET').trim().toUpperCase();
  ...
  const body =
    method === 'POST'
      ? boundedJsonBody(intent.requestBody)
      : undefined;
  if (
  ...
  if (challengeEvidence !== undefined) {
    ...
    if (challengeEvidence.walletInputBinding) {
      ...
      walletBindingMatches =
        ...
        (binding.parameter === 'address' ||
          binding.parameter === 'wallet') &&
    ...
    if (
      challengeEvidence.policyVersion !==
        'kletia_x402_challenge_v1' ||
