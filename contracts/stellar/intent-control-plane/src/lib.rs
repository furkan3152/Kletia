...
#[contract]
pub struct KletiaIntentControlPlane;
...
#[contractevent(topics = ["workflow_committed"])]
#[derive(Clone, Debug, Eq, PartialEq)]
...
#[contractevent(topics = ["workflow_finalized"])]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorkflowFinalized {
    #[topic]
    pub owner: Address,
    #[topic]
    pub nonce: u64,
...
#[contractevent(topics = ["workflow_cancelled"])]
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl KletiaIntentControlPlane {
    ...
    pub fn verifier_registry(env: Env) -> Result<Address, ControlPlaneError> {
        env.storage()
            .instance()
            .get(&DataKey::VerifierRegistry)
            .ok_or(ControlPlaneError::NotInitialized)
    }
    ...
    pub fn get(env: Env, owner: Address, nonce: u64) -> Option<WorkflowRecord> {
        env.storage()
            .persistent()
    ...
    pub fn commit(
        ...
        require_hash(&policy_root)?;
        require_hash(&protocol_registry_root)?;
        require_hash(&asset_registry_root)?;
        require_hash(&recipient_policy_root)?;
        require_hash(&nullifier)?;
        require_hash(&execution_context_commitment)?;
        if verifier_version == 0 {
            return Err(ControlPlaneError::InvalidVerifierVersion);
        }
        if proof.is_empty() || proof.len() > 16_384 {
            return Err(ControlPlaneError::InvalidProof);
        }
        ...
        // The control plane constructs the exact public input order itself.
        // A caller cannot present a valid proof for one workflow/policy and
        // commit different roots, lane, expiry or nullifier here.
        let mut public_inputs = Vec::new(&env);
        public_inputs.push_back(workflow_root.clone());
        public_inputs.push_back(policy_root.clone());
        public_inputs.push_back(protocol_registry_root);
        public_inputs.push_back(asset_registry_root);
        public_inputs.push_back(recipient_policy_root);
        public_inputs.push_back(u32_scalar(
            &env,
            match lane {
        ...
        public_inputs.push_back(u32_scalar(&env, execution_expires_at_ledger));
        public_inputs.push_back(nullifier.clone());
        public_inputs.push_back(execution_context_commitment);
        ...
        if nonce != expected {
            return Err(ControlPlaneError::InvalidNonce);
        }
        let next = nonce
            .checked_add(1)
            .ok_or(ControlPlaneError::NonceExhausted)?;
        let workflow_key = DataKey::Workflow(owner.clone(), nonce);
        let nullifier_key = DataKey::Nullifier(nullifier.clone());
        if env.storage().persistent().has(&workflow_key) {
            return Err(ControlPlaneError::WorkflowAlreadyExists);
        }
        if env.storage().persistent().has(&nullifier_key) {
            return Err(ControlPlaneError::NullifierAlreadyUsed);
        }
        ...
        // Effects are reserved before the only external call. A reviewed
        // verifier cannot re-enter this workflow/nullifier while verification
        // is in progress. Soroban rolls all reservations back atomically if
        // the verifier errors, panics or rejects the proof; the negative proof
        // test below exercises that rollback boundary.
        let verifier_registry = Self::verifier_registry(env.clone())?;
        let accepted = PolicyVerifierRegistryClient::new(&env, &verifier_registry).verify(
            &verifier_version,
            &public_inputs,
            &proof,
        );
        if !accepted {
            return Err(ControlPlaneError::PolicyProofRejected);
        }
    ...
    pub fn finalize(
        ...
        let mut record: WorkflowRecord = env
            .storage()
            .persistent()
            .get(&key)
            .ok_or(ControlPlaneError::WorkflowNotFound)?;
        if record.status != WorkflowStatus::Active {
            return Err(ControlPlaneError::WorkflowNotActive);
        }
        if env.ledger().sequence() >= record.receipt_close_by_ledger {
            return Err(ControlPlaneError::ReceiptWindowClosed);
        }
...
fn require_hash(value: &BytesN<32>) -> Result<(), ControlPlaneError> {
    if value.to_array() == [0_u8; 32] {
        return Err(ControlPlaneError::InvalidHash);
    }
    Ok(())
}
...
fn validate_deadlines(
    ...
    if execution_expiry <= current {
        return Err(ControlPlaneError::InvalidExpiry);
    }
    if receipt_close <= execution_expiry {
        return Err(ControlPlaneError::InvalidReceiptDeadline);
    }
    if retention < receipt_close {
        return Err(ControlPlaneError::InvalidRetention);
    }
    let ttl = retention
        .checked_sub(current)
        .ok_or(ControlPlaneError::InvalidRetention)?;
    if ttl == 0 {
        return Err(ControlPlaneError::InvalidRetention);
    }
    if ttl > env.storage().max_ttl() {
        return Err(ControlPlaneError::BeyondMaximumTtl);
    }
    Ok(())
}
