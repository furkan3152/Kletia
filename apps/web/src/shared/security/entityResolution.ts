...
export const collectEntityResolutionWarnings = (
  evidence: Pick<IntentEntityResolution, "warnings" | "assets" | "recipients">,
): string[] =>
  Array.from(
    new Set([
      ...evidence.warnings,
...
const SECURITY_PROVIDERS = new Set(["Kletia reviewed registry", "GoPlus"]);
...
const isBoundedString = (value: unknown, min = 1, max = 240): value is string =>
  typeof value === "string" &&
  value.length >= min &&
  value.length <= max &&
  value.trim() === value;
...
const isIsoTimestamp = (value: unknown): value is string => {
  ...
  return (
    Number.isFinite(parsed) && parsed <= Date.now() + ISO_TIME_MAX_FUTURE_MS
  );
};
...
export const isEntityClarification = (
  ...
  const expectedField =
    value.kind === "asset"
      ...
      : value.kind === "workflow"
        ? typeof value.field === "string" &&
          /^workflowSteps\.\d+\.(tokenIn|tokenOut|collateralToken|borrowToken)$/u.test(value.field)
...
const isRecipientEvidence = (value: unknown): boolean => {
  ...
  if (
    ...
    (value.warning !== undefined && !isBoundedString(value.warning, 1, 500)) ||
