...
} from '../intent/baseIntentRouterV2.js';

export const BASE_SWAP_EXECUTION_MODE_ENV =
    'BASE_SWAP_EXECUTION_MODE';
export const BASE_INTENT_V2_ROUTER_ENV =
...
export function parseBaseIntentV2DeploymentEvidence(
    ...
    const encoded =
        environment[BASE_INTENT_V2_EVIDENCE_ENV]?.trim();
...
export function resolveConfiguredBaseSwapExecution(
    ...
    const configuredMode =
        environment[BASE_SWAP_EXECUTION_MODE_ENV]?.trim() ||
        'legacy_v1';
    ...
    const evidence =
        configuredMode === 'intent_v2'
            ? deploymentEvidence ||
              parseBaseIntentV2DeploymentEvidence(environment)
            : undefined;
    return resolveBaseSwapExecutionConfig(
...
export function configuredBaseIntentV2AddressManifest(
    ...
    try {
        const router = getAddress(
            environment[BASE_INTENT_V2_ROUTER_ENV]!.trim(),
        );
        const evidence =
            parseBaseIntentV2DeploymentEvidence(environment);
        ...
        return {
            ...
            policyDependencies: [
                ...new Map(
                    policyDependencies.map((address) => [
                        address.toLowerCase(),
                        address,
                    ]),
                ).values(),
