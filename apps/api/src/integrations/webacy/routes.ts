...
import { resolveStrictRequestNetwork } from "../../shared/http/network.js";
import { parseStrictRiskScore } from "../../shared/security/riskScore.js";
import {
...
async function scanBaseAddress(address: Address) {
  ...
  const isDelegatedAccount = isEip7702DelegationDesignator(bytecode);
  const risk: any = isContract && !isDelegatedAccount
    ? await withTimeout(
  ...
  return {
    ...
    address,
    isContract,
    accountType: isDelegatedAccount
