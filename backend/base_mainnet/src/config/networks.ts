...
import {
  ...
  COMPOUND_V3_BASE,
  MOONWELL_BASE,
} from './baseProtocols.js';
import {
  configuredBaseIntentV2AddressManifest,
} from './baseIntentRouterV2Environment.js';
import {
  configuredBaseTokenDeploymentTarget,
} from './baseLaunchFactoryV2Environment.js';
...
const BASE_INTENT_V2_ADDRESS_MANIFEST =
  configuredBaseIntentV2AddressManifest(process.env);
const ACROSS_SPOKE_POOL =
...
export const BASE_CONTRACTS = Object.freeze({
  ...
  basenameL2Resolver: getAddress(
    '0x426fA03fB86E510d0Dd9F70335Cf102a98b10875',
  ),
  x402Factory: getAddress(
...
const BASE_ACTION_TARGETS: Readonly<
...
> = {
  ...
  basename_register: targetSet(
    BASE_CONTRACTS.basenameRegistrarController,
  ),
  basename_renew: targetSet(
    BASE_CONTRACTS.basenameRegistrarController,
  ),
...
const BASE_STATIC_TARGETS = new Set(
  [
    ...Object.values(BASE_ACTION_TARGETS).flatMap(
      (targets) => [...targets],
    ),
    ...(
...
export function isNetworkTargetAllowed(
  ...
  if (network === 'arc') {
    ...
    const actionTargets =
      ARC_ACTION_TARGETS[action.trim().toLowerCase()];
  ...
  if (normalizedAction === 'deploy_token') {
    const selectedFactory =
      configuredBaseTokenDeploymentTarget(process.env);
    return (
      selectedFactory !== null &&
      selectedFactory.toLowerCase() === lower
    );
  }
  if (!action) {
    const selectedFactory =
