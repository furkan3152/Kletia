...
function routeError(res: Response, error: unknown) {
  const known =
    error instanceof BaseMcpRouteError || error instanceof BaseX402IntentError;
    ...
    return res.json({
      ...
      capabilities: {
        ...
        x402Prepare: {
          ...
          nativeTools: ["initiate_x402_request", "complete_x402_request"],
        },
      },
...
router.get("/x402/prepare", (req, res) => {
  try {
    ...
    const maxPayment = scalarQuery(req.query.maxPayment, "maxPayment", {
      required: true,
    })!;
