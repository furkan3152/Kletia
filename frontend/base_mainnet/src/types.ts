...
import { isBaseIntentRouterV2SwapBinding } from './security/baseIntentV2Route';
...
export const isBaseX402Service = (
  value: unknown,
): value is BaseX402Service => {
  if (
    !isObjectRecord(value) ||
...
export const isBaseX402Search = (
  value: unknown,
): value is BaseX402Search =>
  isObjectRecord(value) &&
...
export const isBaseMcpX402Plan = (
  value: unknown,
): value is BaseMcpX402Plan => {
  if (
    !isObjectRecord(value) ||
    value.version !== 1 ||
    value.network !== 'base' ||
    ...
    value.complete.requestIdFrom !==
      'initiate_x402_request.requestId'
  ) {
    return false;
  }
    ...
    return (
      ...
      (
        value.secondaryAmountPolicy === 'live_reserve_ratio'
          ...
          : value.secondaryAmountPolicy ===
              'user_maximum_input_cap' &&
            isPositiveAtomicString(
              value.secondaryAmountCapAtomic,
            ) &&
            BigInt(value.amountBAtomic) <=
              BigInt(value.secondaryAmountCapAtomic)
      )
    );
  }
  return (
...
export const isBaseLiquidityRoutingResponse = (
  ...
  const action =
    ...
    value.actionType === 'remove_liquidity'
      ? value.actionType
      : value.action === 'add_liquidity' ||
          value.action === 'remove_liquidity'
        ? value.action
        : undefined;
  ...
  const routes =
    value.allRoutes as Array<Record<string, unknown>>;
  ...
  const observedBlocks = routes.map((route) =>
    (route.poolEvidence as BaseLiquidityPoolEvidence).observedBlock);
  if (
  ...
  return (
    evidence.rankedRoutes as Array<Record<string, unknown>>
  ).every((ranked, index) => {
    const route = routes[index];
    const pool =
      route.poolEvidence as BaseLiquidityPoolEvidence;
    return (
      ranked.rank === index + 1 &&
...
export const isBaseSwapRoutingEvidence = (
      ...
      (
        ...
        !ranking.rankedRoutes.every(
          (route, index) =>
            ...
            route.rank === index + 1 &&
            typeof route.protocolId === 'string' &&
            /^[a-z0-9][a-z0-9-]{1,63}$/.test(
              route.protocolId,
            ) &&
            typeof route.name === 'string' &&
            route.name.length > 0 &&
            isPositiveAtomicString(
              route.guaranteedNetMinimumAtomic,
            ) &&
            isPositiveAtomicString(
              route.quotedGrossAmountAtomic,
            ) &&
  ...
  const diagnosticsAreConsistent =
    ...
    (coverage.totalSuccessfulQuoteReadCount === undefined ||
      coverage.totalSuccessfulQuoteReadCount ===
  ...
  return (
    ...
    ranking.simulationPassedCount +
      ranking.deferredUntilApprovalCount ===
      ranking.eligibleRouteCount
  );
};
...
export const isBaseYieldRankingEvidence = (
  ...
  return value.rankedRoutes.every((route, index) =>
    isObjectRecord(route) &&
    route.rank === index + 1 &&
    typeof route.protocolId === 'string' &&
    /^[a-z0-9][a-z0-9-]{0,63}$/.test(route.protocolId) &&
...
export const isBaseYieldComparisonResponse = (
  ...
  const opportunities =
    value.opportunities as Array<Record<string, unknown>>;
  const validOpportunities = opportunities.every((opportunity) =>
...
export const isArcPortfolioData = (
  value: unknown,
): value is ArcPortfolioData =>
  isObjectRecord(value) &&
  value.network === 'arc' &&
...
export type EntityClarification = {
  kind: 'asset' | 'recipient' | 'protocol';
  code: string;
  field?: EntityAssetField | 'recipient' | 'protocol';
  reference?: string;
  question: string;
...
const compareBaseIntentV2Routes = (
  ...
  const leftEconomics = left.economics as
    | Record<string, unknown>
    | undefined;
  const rightEconomics = right.economics as
    | Record<string, unknown>
    | undefined;
  const leftMinimum =
  ...
  return String(left.adapterData).localeCompare(
    String(right.adapterData),
  );
};
...
export const isBaseIntentRouterV2ResponseBinding = (
  ...
  const expectedPolicy = routes.some(
    ({ adapterKind }) =>
      adapterKind === 'uniswap_v3_swaprouter02',
  )
    ? 'kletia_base_intent_v2_typed_adapter_v2'
    : 'kletia_base_intent_v2_typed_adapter_v1';
  ...
  if (
    ...
    !quoteCoverage ||
    quoteCoverage.totalQuotedRouteCount <
      coverage.quotedRouteCount ||
  ...
  const sharedIntentFieldsAreValid = routes.every((route) => {
    ...
    return (
      ...
      String(route.quoteExpiresAt) ===
        String(first.quoteExpiresAt) &&
  ...
  if (
    ...
    routes.some(
      (route, index) =>
        index > 0 &&
        compareBaseIntentV2Routes(routes[index - 1], route) > 0,
    )
  ) {
    return false;
  }
  ...
  if (
    ...
    !rankedRoutes.every((ranked, index) => {
      ...
      const economicsRecord = economics as
        | Record<string, unknown>
        | undefined;
      return (
    ...
    (
      nativeInput
        ...
        : sameCanonicalAddress(
            response.tokenInAddress,
            firstIntent.tokenIn,
          )
    ) &&
    String(response.quoteExpiresAt) ===
      String(first.quoteExpiresAt)
  );
};
...
export const hasExecutableIntentActionBinding = (
  ...
  if (
    hasBaseIntentV2Marker(response) ||
    hasBaseIntentV2RouteMarker(route)
  ) {
    return (
