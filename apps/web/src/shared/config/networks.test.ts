...
describe("network execution profiles", () => {
  ...
  it("materializes recipient examples only from the active wallet", () => {
    ...
    for (const prompt of walletPrompts) {
      const editable = materializeIntentExample(prompt);
      expect(editable).toContain(EDITABLE_RECIPIENT_ADDRESS);
      expect(editable).not.toContain(ACTIVE_WALLET_ADDRESS);
      expect(editable).not.toMatch(/0x[a-fA-F0-9]{40}/u);
