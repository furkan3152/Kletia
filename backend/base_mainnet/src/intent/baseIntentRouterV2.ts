...
const V3_MAX_HOPS = 4;
const V3_MAX_PATH_BYTES =
    V3_ADDRESS_BYTES + V3_MAX_HOPS * V3_NEXT_HOP_BYTES;
...
const PUBLIC_ERRORS = {
    ...
    BASE_INTENT_V2_CONFIG_INVALID: {
        message:
            'Base V2 işlem yapılandırması eksik veya doğrulanamadı.',
        statusCode: 503,
    },
    ...
    BASE_INTENT_V2_TIME_INVALID: {
        message:
            'Base V2 niyet zaman aralığı geçersiz veya bir saati aşıyor.',
        statusCode: 400,
    },
...
export type BaseIntentV2AdapterEvidence =
    | BaseIntentV2UniV2AdapterEvidence
    | BaseIntentV2UniV3AdapterEvidence;
...
export type ReviewedBaseIntentV2ExactInputRoute =
    | ReviewedUniV2ExactInputRoute
    | ReviewedUniV3ExactInputRoute;
...
function configError(): never {
    throw new BaseIntentV2PlanError(
        'BASE_INTENT_V2_CONFIG_INVALID',
    );
}
...
function validateAdapterEvidence(
    ...
    const expectedKeys = raw.kind === 'uniswap_v3_swaprouter02'
    ...
    const adapterConfigurationHash =
        checkedBytes32(raw.adapterConfigurationHash);
    ...
    if (
        ...
        [
            ...
            sameAddress(address, wrappedNative) ||
            sameAddress(address, router)
        ) ||
...
function validateDeploymentEvidence(
    ...
    const observedAtBlock = checkedConfigUint(
        raw.observedAtBlock,
        UINT256_MAX,
    );
    ...
    const wrappedNativeCodehash =
        checkedBytes32(raw.wrappedNativeCodehash);
    if (
    ...
    if (
        ...
        adapters.some(
            (adapter) =>
                adapter.kind !== 'uniswap_v2_compatible',
        )
    ) {
        configError();
    }
    ...
    for (const adapter of adapters) {
        ...
        if (
            protocolKinds.has(
                `${adapter.kind}:${adapter.protocolId}`,
            ) ||
 ...
 */
export function decodeUniV3PackedPath(
    value: unknown,
): DecodedUniV3PackedPath {
    ...
    const hopCount =
        (byteLength - V3_ADDRESS_BYTES) / V3_NEXT_HOP_BYTES;
...
function validateRouteShape(
    ...
    const quotedAmountOut = checkedRouteUint(
    ...
    const normalizedTokenOut =
        tokenOut === NATIVE_TOKEN_SENTINEL
            ...
            : tokenOut;
    if (
        sameAddress(normalizedTokenIn, normalizedTokenOut)
    ) {
        routeError();
    }
    ...
    if (route.kind === 'uniswap_v2_compatible') {
        ...
        const path = route.path.map((token) =>
            checkedRouteAddress(token),
        );
...
function checkedIntentTimes(input: BuildBaseIntentV2SwapPlanInput): {
    ...
    try {
        ...
        const issuedAt = checkedRouteUint(
            input.issuedAt,
            UINT48_MAX,
        );
        const validAfter = checkedRouteUint(
            input.validAfter,
            UINT48_MAX,
        );
        const deadline = checkedRouteUint(
            input.deadline,
            UINT48_MAX,
        );
...
export function buildBaseIntentV2SwapPlan(
    ...
    const checked = validateRouteShape(
        input.route,
        config.deployment,
    );
    ...
    if (
        executor !== NATIVE_TOKEN_SENTINEL &&
        !sameAddress(executor, owner)
    ) {
        routeError();
    }
    ...
    if (
        forbiddenRecipients.some((address) =>
            sameAddress(address, recipient),
        )
    ) {
        routeError();
    }
    const times = checkedIntentTimes(input);
    const maxFeeBps = checkedFeePolicy(
        input.maxFeeBps,
        config.deployment.feeBps,
    );
    ...
    const grossMinimumAfterSlippage =
        ...
        (
            BPS_DENOMINATOR -
            BigInt(checked.route.slippageBps)
        ) /
        BPS_DENOMINATOR;
    const estimatedFeeAtObservedRate =
        grossMinimumAfterSlippage *
        BigInt(config.deployment.feeBps) /
        BPS_DENOMINATOR;
    ...
    if (input.minimumNetAmountOut !== undefined) {
        userMinimumNetAmountOut = checkedRouteUint(
            input.minimumNetAmountOut,
        );
        ...
        if (
            ...
            maximumQuotedNetAfterSignedFeeCap
        ) {
            routeError();
        }
    }
    ...
    if (
        grossMinimumAfterSlippage === 0n ||
        netMinimumAmountOut === 0n
    ) {
        routeError();
    }
    ...
    const isNativeInput =
        checked.route.tokenIn === NATIVE_TOKEN_SENTINEL;
    ...
    return {
        ...
        calldata,
        value: isNativeInput
            ? checked.route.amountIn.toString()
            : '0',
        adapterData,
        intent: {
            owner,
        ...
        economics: {
            quotedGrossAmountOut:
                checked.route.quotedAmountOut.toString(),
            grossMinimumAfterSlippage:
                grossMinimumAfterSlippage.toString(),
            estimatedFeeAtObservedRate:
                estimatedFeeAtObservedRate.toString(),
            maximumFeeAtSignedCap:
                maximumFeeAtSignedCap.toString(),
            netMinimumAmountOut: netMinimumAmountOut.toString(),
            userMinimumNetAmountOut:
                userMinimumNetAmountOut?.toString() ?? null,
        ...
        configEvidence: {
            ...
            adapterKind: checked.adapter.kind,
            observedAtBlock:
                config.deployment.observedAtBlock.toString(),
            routerCodehash: config.deployment.routerCodehash,
            wrappedNativeCodehash:
                config.deployment.wrappedNativeCodehash,
            adapterConfigHash:
                checked.adapter.adapterConfigHash,
            adapterConfigurationHash:
                checked.adapter.adapterConfigurationHash,
        },
    };
}
