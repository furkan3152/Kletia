...
export const IntentSchema = z.object({
  ...
  protocol: z
    ...
    .optional(),
  objective: z
    .enum(["best_output", "best_rate", "lowest_borrow_cost", "lowest_risk"])
    .optional(),
  ...
  destinationChain: z
    .any()
    .transform((v) => (v == null ? undefined : String(v).trim().toLowerCase()))
    .optional(),
  ...
  slippage: z
    .any()
    .transform((v) => (v == null ? "1" : String(v).replace("%", "")))
    .optional(),
  recipient: z
    .any()
    .transform((v) => (v == null ? undefined : String(v).trim()))
  ...
  transferSpeed: z
    .any()
    .transform((v) => (v == null ? undefined : String(v).trim().toUpperCase()))
    .optional(),
  serviceQuery: z
    .any()
    .transform((v) => (v == null ? undefined : String(v).trim()))
...
function normalizeAssetMention(
...
function arcActionSupportsMax(action: string, tokenIn: unknown): boolean {
  if (action === "swap") {
    return (
      String(tokenIn || "")
        .trim()
  ...
  return new Set(["unstake", "remove_liquidity", "lending_withdraw"]).has(
    action,
  );
}
...
function normalizeParsedAction(action: unknown, network: NetworkId): string {
  const normalized = String(action || "unknown")
    ...
    .toLowerCase();
  return network === "arc"
    ? ARC_ACTION_ALIASES[normalized] || normalized
    : normalized;
}

/**
 * A positive financial instruction may also describe the wallet-approval
 * boundary. That is a request to prepare a reviewable route, not a request to
 * suppress planning altogether. Execution still remains impossible without
 * the normal simulation and wallet-signature gates.
 */
function hasApprovalBoundPlanningRequest(prompt: string): boolean {
  ...
  const positiveInstruction =
    /^(?:swap|stake|unstake|lend|supply|deposit|withdraw|borrow|repay|bridge|transfer|send|pay|buy|sell|convert|add\s+liquidity|remove\s+liquidity|call|find|discover|prepare|create|atomically\s+pay|takasla|çevir|cevir|yatır|yatir|çek|cek|borçlan|borclan|gönder|gonder|öde|ode|satın\s+al|satin\s+al)\b/iu.test(
      normalized,
    );
  if (!positiveInstruction) return false;

  return (
    /\b(?:before|prior\s+to)\s+(?:(?:my|wallet|user|the\s+user['’]?s)\s+)?(?:approval|signature|execution)\b/iu.test(
      normalized,
    ) ||
    /\b(?:do\s+not|don['’]?t|must\s+not)\s+(?:execute|submit|broadcast|send|pay)\b[^,;:.!?\n]{0,120}\b(?:until|unless)\s+(?:i|the\s+user)\s+(?:explicitly\s+)?approve\b/iu.test(
      normalized,
    )
  );
}
...
function hasNonExecutionSpeechAct(prompt: string): boolean {
  const normalized = prompt
    ...
    .replace(/\u0307/gu, "");
  if (!normalized) return false;
  if (hasApprovalBoundPlanningRequest(prompt)) return false;
  if (
    /\b(?:portfolio|portföy|portfoy)\b/iu.test(normalized) &&
    /\bwithout\s+(?:sending|submitting|broadcasting)\s+(?:a\s+)?transaction\b/iu.test(
      normalized,
    )
  ) {
    return false;
  }
  // Borrow-capacity is an explicitly read-only product action. It still goes
  // through network, asset, wallet and live Aave-state binding, but wording
  // such as "without sending a transaction" must not demote it to chat.
  if (
  ...
  const explicitFinalExecution =
    /(?:^|[,;:.!?]\s*|\b(?:but|however|instead|now|then|ama|ancak|şimdi|simdi)\s+)(?:actually\s+)?(?:execute|submit|broadcast|proceed|do\s+it|go\s+ahead(?:\s+and)?|prepare\s+(?:the\s+)?(?:transaction|route)|işlemi\s+(?:hazırla|hazirla|çalıştır|calistir|gerçekleştir|gerceklestir)|rotayı\s+(?:hazırla|hazirla)|devam\s+et)\s*[.!?]*$/iu.test(
      normalized,
    );
  if (explicitFinalExecution) return false;

  const explicitlyReadOnly =
    /\b(?:quote|preview|simulation|simulate|estimate|example)\s+only\b|\b(?:simulate|preview|estimate)\b[^,;:.!?\n]{0,120}\bonly\b|\bonly\s+(?:quote|preview|simulate|estimate)\b|\bjust\s+(?:quote|preview|simulate|estimate|explain)\b|\b(?:without|with\s+no)\s+(?:executing|execution|submitting|submission|broadcasting|sending\s+(?:a\s+)?transaction|swapping|trading|buying|selling|staking|lending|borrowing)\b|\bdo\s+not\s+(?:actually\s+)?(?:execute|submit|broadcast)(?:\s+(?:it|that|this))?(?:\s+yet)?\b|\bnot\s+asking\s+(?:you\s+)?to\b|\b(?:sadece|yalnızca|yalnizca)\s+(?:teklif|önizleme|onizleme|simülasyon|simulasyon|açıklama|aciklama)\b|\b(?:gerçekleştirmeden|gerceklestirmeden|göndermeden|gondermeden)\b|\b(?:henüz|henuz)\s+(?:gerçekleştirme|gerceklestirme|gönderme|gonderme|yapma)\b/iu.test(
      normalized,
    );
  const informational =
    /^(?:show\s+me\s+how|how\s+(?:do|can|would)\s+i|could\s+you\s+(?:explain|show|tell)|can\s+you\s+explain|explain\s+(?:how|what)|tell\s+me\s+how|what\s+(?:happens|would\s+happen|is\s+the\s+result)\s+if|what\s+if|[iı]\s+(?:am|was)\s+wondering\s+(?:what|how)|show\s+me\s+an?\s+example|give\s+me\s+an?\s+example)\b/iu.test(
      normalized,
    ) ||
    /\b(?:example|örnek|ornek)\s+please\s*[.!?]*$/iu.test(normalized) ||
    /^(?:nasıl|nasil)\s+.+\b(?:yapılır|yapilir|olur)\b/iu.test(normalized) ||
    /\b(?:nasıl\s+yapılır|nasil\s+yapilir|yaparsam\s+ne\s+olur|örnek\s+(?:göster|ver)|ornek\s+(?:goster|ver)|nasıl\s+çalıştığını\s+açıkla|nasil\s+calistigini\s+acikla)\b/iu.test(
      normalized,
    );
  const hypothetical =
    /^(?:suppose|supposing|imagine|assuming|what\s+if|maybe|perhaps)\b|\b[iı]\s+(?:might|may)\b|\b[iı]\s+(?:am|was)\s+(?:considering|thinking\s+about)\b|^(?:belki|varsayalım|varsayalim|farz\s+edelim)\b/iu.test(
      normalized,
    );
...
function hasExplicitTransactionNegation(prompt: string): boolean {
  ...
  if (
    /^(?:no(?:\s+(?:thanks|thank\s+you))?|cancel|stop|don['’]?t|never\s+mind|forget\s+(?:it|that)|hayır(?:\s+teşekkürler)?|hayir(?:\s+tesekkurler)?|yapma|etme|iptal|boşver|bosver|vazgeç|vazgec|vazgeçtim|vazgectim)[.!?]?$/iu.test(
      lower,
    )
  ) {
    return true;
  ...
  const hasTrailingCancellation =
    /(?:^|[,;:.!?–—-]\s*|\bbut\s+|\bama\s+)(?:actually\s+)?(?:[iı]\s+(?:(?:do\s+not|don['’]?t)\s+want\s+(?:you\s+)?to\s+do\s+(?:it|that|this)|refuse|decline)|no(?:\s+(?:thanks|thank\s+you))?|(?:please\s+)?(?:do\s+not|don['’]?t)(?:\s+(?:actually\s+)?(?:execute|do|send|submit|broadcast)(?:\s+(?:it|that|this))?)?(?:\s+yet)?|not\s+(?:now|anymore)|cancel(?:\s+(?:that|it|this))?|stop(?:\s+(?:that|it|this))?|skip(?:\s+(?:that|it|this))?|never\s+mind|forget\s+(?:about\s+)?(?:it|that|this)|disregard\s+(?:it|that|this)|[iı]\s+changed\s+my\s+mind|hayır(?:\s+teşekkürler)?|hayir(?:\s+tesekkurler)?|iptal(?:\s+et)?|boşver|bosver|vazgeç|vazgec|(?:henüz|henuz)\s+(?:gerçekleştirme|gerceklestirme|gönderme|gonderme|yapma)|yapma|etme)\s*[.!?]*$/iu.test(
      lower,
    );
  if (containsTransactionAction && hasTrailingCancellation) {
    return true;
  }
  if (
  ...
  if (
    transactionSignal.test(lower) &&
    /\b(?:istemiyorum|istemiyoruz|vazgeçtim|vazgectim|iptal\s+et)\b/iu.test(
      lower,
    )
  ) {
    return true;
  ...
  return (
    /\b(?:swap|stake|unstake|lend|borrow|bridge|takas|transfer|register|renew)\s+(?:yapma|etme)\b/iu.test(
      lower,
    ) ||
    /\blikidite\s+(?:ekleme|çıkarma|cikarma|çekme)\b/iu.test(lower) ||
    /\bborç\s+(?:alma|verme|ödeme)\b(?:\s+(?:sakın|sakin|lütfen|lutfen))?(?:[.!]|$)/iu.test(
      lower,
    ) ||
    /\b(?:satın\s+alma|alma|satma)\b/iu.test(lower) ||
    /(?:^|[^\p{L}\p{N}_])(?:swapla|takasla|çevir|cevir|al|sat|stake|unstake|bridge|gönder|gonder|yatır|yatir|çek|cek|borçlan|borclan|ekle|çıkar|cikar)(?:ma|me|mayalım|meyelim|mayalim|meyelim|mayın|meyin|mayin|masın|mesin|masin)(?=$|[^\p{L}\p{N}_])/iu.test(
      lower,
    ) ||
    /\b(?:yatırma|yatirma|çekme|cekme|ödeme|odeme|gönderme|gonderme|kaydetme|yenileme|uzatma)\b(?:\s+(?:sakın|sakin|lütfen|lutfen))?(?:[.!]|$)/iu.test(
      lower,
    ) ||
    /(?:^|[^\p{L}\p{N}_])(?:almak|satmak|swaplamak|takaslamak|çevirmek|cevirmek|stake\s+etmek|unstake\s+etmek|borçlanmak|borclanmak|borç\s+almak|borc\s+almak|yatırmak|yatirmak|çekmek|cekmek|göndermek|gondermek|ödemek|odemek|kaydetmek|yenilemek|uzatmak)\s+istem(?:iyorum|iyoruz)(?=$|[^\p{L}\p{N}_])/iu.test(
      lower,
    ) ||
    /(?:^|[^\p{L}\p{N}_])(?:çevril|cevril|swaplan|takaslan)(?:me|ma|mesin|masın|masin|meyelim|mayalım|mayalim|meyin|mayın|mayin)(?=$|[^\p{L}\p{N}_])/iu.test(
      lower,
    )
  );
}
...
function isExplicitlyNegatedAssetOccurrence(
  ...
  const before = text.slice(
    Math.max(0, occurrence.start - 64),
    occurrence.start,
  );
  const after = text.slice(occurrence.end, occurrence.end + 64);
  return (
    /(?:\b(?:not|no|without|except|exclude|avoid)\b|hariç|haric|kullanma|kullanmayın|kullanmayin)\s*(?:(?:the|a|an)\s+)?(?:(?:token|asset|coin)\s+)?$/iu.test(
      before,
    ) ||
    /(?:\b(?:do\s+not|don['’]?t)\s+(?:use|select|choose|include|route\s+through)\s+)$/iu.test(
      before,
    ) ||
    /^\s*(?:['’]?(?:yi|yı|yu|yü|i|ı|u|ü))?\s*(?:değil|degil|hariç|haric|yerine|olmasın|olmasin|kullanma|kullanmayın|kullanmayin)\b/iu.test(
      after,
    ) ||
...
function hasPromptBoundPrimaryAmount(
  ...
  const token = String(
    intent.action === "liquid_stake" ? intent.tokenOut : intent.tokenIn || "",
  ).trim();
  ...
  if (amount.toUpperCase() === "MAX") {
    ...
    return tokenReferences.some((tokenReference) => {
      const escapedToken = escapeRegExp(tokenReference).replace(/\s+/g, "\\s+");
      return new RegExp(
        `(?:^|[^a-zA-Z0-9])` +
...
function explicitPromptAddresses(text: string): Set<string> {
  return new Set(
    Array.from(text.matchAll(EXPLICIT_EVM_ADDRESS_PATTERN)).map((match) =>
      match[0].toLowerCase(),
    ),
  );
}
...
function hasPromptBoundTransfer(
  ...
  return (
    explicitTransferPairs(text).get(address)?.has(normalizedAmount) === true
  );
}
...
function hasPromptBoundJsonObject(value: unknown, text: string): boolean {
...
function hasPromptBoundAction(action: string, text: string): boolean {
  const rules: Record<string, RegExp> = {
    ...
    unstake:
      /\b(?:unstake|unlock|stake(?:den|ten)\s+çık|stake(?:den|ten)\s+cik)\b/iu,
    claim_rewards:
    ...
    borrow_capacity:
      /(?:\b(?:how\s+much|maximum|max|capacity|available|safe(?:ly)?)\b[^,;:.!?\n]{0,80}\b(?:borrow|loan)\b|\b(?:borrow|loan)\b[^,;:.!?\n]{0,80}\b(?:capacity|maximum|max|available|safe(?:ly)?))\b/iu,
    ...
    lending_deposit:
      /(?:\b(?:lending|collateral)\b[^,;:.!?\n]{0,48}\b(?:deposit|supply|add)|\b(?:deposit|supply|add|place|put|provide)\b[^,;:.!?\n]{0,64}\b(?:lending|collateral)\b|\bteminat\b[^,;:.!?\n]{0,32}\b(?:ekle|yatır))/iu,
    lending_withdraw:
    ...
    allora_prediction: /\b(?:allora|predict|prediction|forecast|tahmin)\b/iu,
    agent_action:
    ...
    workflow:
      /^(?=[\s\S]*(?:\b(?:then|after|next|and then)\b|\band\s+(?:swap|lend|supply|borrow|repay|withdraw)\b|sonra|ardından|ardindan))(?=[\s\S]*(?:\b(?:bridge|swap|lend|supply|borrow|repay|withdraw)\b|köprü|kopru|borç|borc|yatır|yatir|çek|cek))[\s\S]*$/iu,
    policy_agent:
...
function hasPromptBoundBaseAction(action: string, text: string): boolean {
  ...
  if (
    action === "swap" &&
    (/\b(?:ile|karşılığında|karsiliginda)\b[^,;:.!?\n]{0,48}\b(?:al|sat)\b/iu.test(
      text,
    ) ||
      /(?<!borç )(?<!borc )(?<!kredi )\b(?:al|sat)\b/iu.test(text))
  ) {
    return true;
  }
  if (action === "lend" && /\b(?:yatır|yatir)\b/iu.test(text)) {
    return true;
  }
  if (
    action === "remove_liquidity" &&
    /likidite(?:yi|yı|u|ü|den|dan|inden|ından)?[^,;:.!?\n]{0,48}(?:çıkar|cikar|çek|cek|kaldır)/iu.test(
      text,
    )
  ) {
    return true;
...
function hasPromptBoundAsset(
  ...
  const candidates = [asset, ...assetAliasesForSymbol(network, asset)].filter(
  ...
  if (role === "tokenOut") {
    ...
    if (intent.action === "liquid_unstake" && asset.toUpperCase() === "ETH") {
      return true;
    }
    if (
      intent.minimumOutput !== undefined &&
      hasPromptBoundMinimumOutput(intent.minimumOutput, asset, text, network)
    ) {
      return true;
    }
...
function hasPromptBoundRecipientIdentity(
  value: unknown,
  text: string,
): boolean {
...
function hasPromptBoundDestination(
  ...
  const phrase = destination.split("-").map(escapeRegExp).join("[\\s_-]+");
  return new RegExp(
    `(?:^|[^\\p{L}\\p{N}_])${phrase}` +
...
function hasPromptBoundAmountForAsset(
  ...
  const references = [asset, ...assetAliasesForSymbol(network, asset)].filter(
    (value, index, values) =>
      value.length <= 128 &&
      values.findIndex(
        (candidate) => candidate.toLowerCase() === value.toLowerCase(),
  ...
  return references.some((reference) => {
    const escapedAsset = escapeRegExp(reference).replace(/\s+/g, "\\s+");
    const patterns = [
      new RegExp(
        `((?:\\d+(?:[.,]\\d+)?|[.,]\\d+))\\s+` +
...
function explicitTransferPairs(text: string): Map<string, Set<string>> {
...
function explicitSlippage(text: string): string | null {
  const match =
    /(?:slippage[^\d\n]{0,16}(\d+(?:[.,]\d+)?)\s*%|(\d+(?:[.,]\d+)?)\s*%\s*slippage)/iu.exec(
      text,
    );
...
function explicitTokenLaunchId(text: string): string | null {
  ...
  const unquoted =
    /\b(?:launch\s*id|launch\s+identifier|lansman\s+kimliği|lansman\s+kimligi)\b\s*(?:is|=|:)?\s*([a-zA-Z0-9][a-zA-Z0-9._:-]{0,127})(?=$|[\s,;.])/iu.exec(
      text,
    );
...
function explicitTransferSpeed(text: string): "FAST" | "SLOW" | null {
  const match =
    /(?:^|[^\p{L}\p{N}_])(FAST|SLOW)(?:\s+(?:mode|modu))?(?=$|[^\p{L}\p{N}_])/iu.exec(
      text,
    );
...
function hasExplicitMaxGasConstraint(text: string): boolean {
  return /(?:max(?:imum)?\s+gas(?:\s+(?:price|fee))?|gas\s+(?:cap|limit)|azami\s+gas)\b/iu.test(
    text,
  );
}

function hasExplicitMaxPriceImpactConstraint(text: string): boolean {
  return /(?:max(?:imum)?\s+price\s+impact|price\s+impact\s+(?:cap|limit)|maksimum\s+fiyat\s+etkisi)\b/iu.test(
    text,
  );
}
...
function baseActionsInClause(text: string): Set<string> {
  ...
  const yieldComparison =
    /\b(?:yield\s+compare|compare\b[^,;:.!?\n]{0,64}\b(?:rates?|yield)|(?:apy|apr)\b|(?:getiri|faiz)[^,;:.!?\n]{0,64}(?:karşılaştır|karsilastir)|(?:karşılaştır|karsilastir)[^,;:.!?\n]{0,64}(?:getiri|faiz))\b/iu.test(
      text,
    );
  const basename =
    /\.base\.eth\b/iu.test(text) &&
    /\b(?:register|buy|purchase|renew|extend|kaydet|kayıt\s+et|satın\s+al|yenile|uzat)\b/iu.test(
      text,
    );
  const deployToken = hasPromptBoundBaseAction("deploy_token", text);
  const addLiquidity = hasPromptBoundBaseAction("add_liquidity", text);
  const removeLiquidity = hasPromptBoundBaseAction("remove_liquidity", text);
  const liquidStake = hasPromptBoundBaseAction("liquid_stake", text);
  const liquidUnstake = hasPromptBoundBaseAction("liquid_unstake", text);
  ...
  if (!yieldComparison && hasPromptBoundBaseAction("borrow", text)) {
    actions.add("borrow");
  }

  const explicitLendingAction =
    /\b(?:lend|earn|borç\s+ver|faize\s+yatır|mevduat(?:a)?\s+yatır)\b/iu.test(
      text,
    );
  const supplyAction = /\bsupply\b/iu.test(text);
  const genericDeposit = /\b(?:deposit|yatır)\b/iu.test(text);
  if (
    !yieldComparison &&
  ...
  if (hasPromptBoundBaseAction("withdraw", text) && !removeLiquidity) {
  ...
  if (hasPromptBoundBaseAction("swap", text) && !basename) {
    actions.add("swap");
  }
...
function explicitlyAllowsUncuratedX402(text: string): boolean {
  return /\b(?:uncurated|unverified|broader\s+(?:catalog|results?)|all\s+(?:catalog|services?)|include\s+unverified|do\s+not\s+limit\s+to\s+curated|curated\s*[:=]\s*false|kürasyonsuz|kurasyonsuz|doğrulanmamış|dogrulanmamis|tüm\s+servisler)\b/iu.test(
    text,
  );
}
...
function enforcePromptBoundIntent(
  ...
  if (intent.action === "memo_send" || intent.action === "official_memo_send") {
  ...
  if (
    (intent.action === "x402_request" || intent.action === "x402_discover") &&
  ...
  if (intent.action === "x402_request") {
    const explicitMethod = /\bPOST\b/iu.test(bindingText)
      ? "POST"
  ...
  const intentMaxGas =
    intent.maxGas === undefined ? null : normalizePromptDecimal(intent.maxGas);
  if (
  ...
  const promptMaxPriceImpactBps = explicitMaxPriceImpactBps(bindingText);
  ...
  if (network === "base") {
    ...
    if (
      intent.excludedProtocols?.some(
        (protocol) =>
          !expected.has(normalizeBaseProtocolId(protocol) || protocol),
      )
    ) {
      return promptBindingFailure(
    ...
    grounded = {
      ...grounded,
      excludedProtocols:
        excludedProtocols.length > 0 ? excludedProtocols : undefined,
    };
  } else if (
    intent.excludedProtocols?.some(
      (protocol) => !isExplicitlyExcludedProtocol(protocol, bindingText),
    )
  ) {
    return promptBindingFailure(
      "Each excluded protocol must be explicitly specified in the user message.",
  ...
  if (network === "base" && intent.protocol) {
    ...
    if (
      ...
      normalizeBaseProtocolId(String(intent.protocol)) !== explicitProtocol
    ) {
  ...
  const objective =
    intent.action === "swap" || intent.action === "stable_swap"
      ? "best_output"
      : intent.action === "borrow" || intent.action === "lending_borrow"
        ? "lowest_borrow_cost"
        : intent.action === "lend"
          ? "best_rate"
          : intent.action === "yield_compare"
            ? /\b(?:borrow|loan)\b|borç|kredi/iu.test(bindingText)
              ? "lowest_borrow_cost"
              : /\b(?:lowest\s+risk|en\s+düşük\s+risk|en\s+dusuk\s+risk)\b/iu.test(
                    bindingText,
                  )
                ? "lowest_risk"
                : "best_rate"
...
function locateBaseTokens(prompt: string): LocatedToken[] {
  const aliases = Object.keys(BASE_TOKEN_REGISTRY).sort(
  ...
  for (const symbol of aliases) {
    ...
    for (const match of prompt.matchAll(matcher)) {
      ...
      const canonical = symbol === "USDBC" ? "USDBC" : symbol;
      if (
        !located.some(
          (entry) =>
            entry.index === index ||
...
function baseProtocolMentions(): BaseProtocolMention[] {
  const products = BASE_PROTOCOL_PRODUCT_NAMES.map(([alias, protocolId]) => ({
    alias,
    protocolId,
    productName: true,
  }));
...
function hasExplicitBaseProtocolContext(
  ...
  const explicitSelectorBefore =
    /(?:\b(?:via|using|use|through|on|from|at)\b|üzerinden|aracılığıyla|araciligiyla|protokol\s+olarak)\s*$/iu.test(
      before,
    );
  const explicitSelectorAfter =
    /^(?:['’]?(?:da|de|ta|te|dan|den|tan|ten)|\s+(?:via|üzerinden|aracılığıyla|araciligiyla)\b)/iu.test(
      after,
    );
  if (explicitSelectorBefore || explicitSelectorAfter) return true;

  const protocolNounBefore =
    /\b(?:protocol|router|dex|exchange|market|vault|pool|protokol|yönlendirici|yonlendirici|borsa|piyasa|havuz)\s*$/iu.test(
      before,
    );
  const protocolNounAfter =
    /^\s+(?:protocol|router|dex|exchange|market|vault|pool|protokol|yönlendirici|yonlendirici|borsa|piyasa|havuz)\b/iu.test(
      after,
    );
  ...
  return /^\s+[^,;:.!?\n]{0,64}\b(?:protocol|router|dex|vault|pool|protokol|havuz(?:undan|dan|den|unda|inde|u)?)\b/iu.test(
    after,
  );
}
...
function attachExplicitBaseExecutionConstraints(
  ...
  const excludedProtocols = extractExplicitlyExcludedBaseProtocols(prompt);
...
function extractAmount(
  ...
  if (
    /\b(max|all|everything|tamam[ıi]|tüm|tümü|hepsi|bakiyemin tamam[ıi])\b/i.test(
      prompt,
    )
  ) {
    return "MAX";
...
function extractMinimumOutputForToken(
  ...
  if (!token) return undefined;
  const prefix = prompt.slice(Math.max(0, token.index - 96), token.index);
  const match =
    /(?:accept|receive)[^,;:.!?\n]{0,48}\bless\s+than\s+(\d+(?:[.,]\d+)?)\s*$/i.exec(
      prefix,
    );
  return match?.[1].replace(",", ".");
}
...
function extractSlippagePercent(prompt: string): string | undefined {
  const match =
    /(?:slippage[^,;:.!?\n]{0,20}?(\d+(?:[.,]\d+)?)\s*%|(\d+(?:[.,]\d+)?)\s*%\s*slippage)/i.exec(
      prompt,
    );
  const raw = (match?.[1] || match?.[2])?.replace(",", ".");
  if (!raw) return undefined;
  const value = Number(raw);
  return Number.isFinite(value) && value > 0 && value <= 50 ? raw : undefined;
}

function detectRiskTolerance(prompt: string): ParsedIntent["riskTolerance"] {
  if (
    /\b(conservative|low[- ]?risk|düşük risk|en güvenli|guvenli)\b/i.test(
      prompt,
    )
  ) {
    return "conservative";
  ...
  if (
    /\b(aggressive|high[- ]?risk|yüksek risk|riskli|maksimum getiri)\b/i.test(
      prompt,
    )
  ) {
    return "aggressive";
...
function extractDurationInDays(prompt: string): number | undefined {
  ...
  const days = /^(?:years?|yıl)/iu.test(unit)
    ? value * 365
...
function parseDeterministicBasenameIntent(prompt: string): ParsedIntent | null {
  const nameMatch =
    /(?:^|[^a-z0-9-])([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)\.base\.eth(?=$|[^a-z0-9.])/i.exec(
      prompt,
    );
  if (!nameMatch) return null;

  const lower = prompt.toLocaleLowerCase("tr-TR");
  const renewRequested = /\b(?:renew|extend)\b|yenile|uzat/iu.test(lower);
  const registerRequested =
    /\b(?:register|buy|purchase)\b|satın\s+al|kaydet|kayıt\s+et/iu.test(lower);
  ...
  if (
    ...
    /\b(?:do not|don't|not)\b.{0,24}(?:register|renew|extend)|(?:yenileme|uzatma|kaydetme|satın\s+alma)\b/iu.test(
      lower,
    )
  ) {
    return null;
  ...
  const action = renewRequested ? "basename_renew" : "basename_register";
...
function parseRawDeterministicBaseIntent(
  ...
  const isYieldComparison =
    /\b(best yield|compare (?:the )?(?:rates|yield)|yield compare|apy|apr|en iyi getiri|en yüksek faiz|en düşük borç faizi|borrow (?:rates?|cost)|borç faiz(?:ini|lerini)? karşılaştır|faiz(?:leri|lerini| oranlar[ıi]n[ıi])? karşılaştır|hangi protokol daha çok kazandır)/i.test(
      lower,
    );
  if (isYieldComparison && tokens.length >= 1) {
    const comparedProtocols = extractNamedBaseYieldProtocols(prompt);
    ...
    const compareBorrow =
      /(?:\b(?:borrow|kredi|lowest borrow|en düşük faiz)\b|borç)/i.test(lower);
    return deterministicIntent({
      action: "yield_compare",
      tokenIn: tokens[0].symbol,
      amount: "0",
      protocol,
      objective: compareBorrow ? "lowest_borrow_cost" : "best_rate",
      riskTolerance,
  ...
  const isRemoveLiquidity =
    /(remove|withdraw|exit|çek|çıkar|kaldır|boz).{0,32}\b(liquidity|likidite|lp|pool|havuz)|\b(liquidity|likidite|lp|pool|havuz)(?:yi|yı|u|ü|dan|den|undan|ünden|ından|inden)?\b.{0,32}(remove|withdraw|exit|çek|çıkar|kaldır|boz)/i.test(
      lower,
    );
  const isAddLiquidity =
    !isRemoveLiquidity &&
    /(add|provide|deposit|ekle|sağla|yatır).{0,32}\b(liquidity|likidite|lp|pool|havuz)|\b(liquidity|likidite|lp|pool|havuz)(?:a|e|ya|ye|una|üne|ına|ine)?\b.{0,32}(add|provide|deposit|ekle|sağla|yatır)/i.test(
      lower,
    );
  if ((isAddLiquidity || isRemoveLiquidity) && tokens.length >= 2 && amount) {
    const secondaryAmount = isAddLiquidity
      ...
      : undefined;
    return deterministicIntent({
      action: isRemoveLiquidity ? "remove_liquidity" : "add_liquidity",
      tokenIn: tokens[0].symbol,
  ...
  const isLiquidStake =
    /\b(liquid stake|liquid staking|likit stake|likit staking|l[r]?t al|l[r]?t edin)\b/i.test(
      lower,
    );
  const liquidToken = tokens.find(({ symbol }) =>
    ["WSTETH", "CBETH", "RETH", "WEETH", "EZETH", "WRSETH"].includes(symbol),
  );
  ...
  const action = /\b(repay|pay back|borç öde|borcu öde|geri öde)\b/i.test(lower)
    ? "repay"
    : /\b(borrow|borç al|kredi çek)\b/i.test(lower)
      ? "borrow"
      : /\b(withdraw|geri çek|pozisyon(?:u)? çek|mevduat(?:ı)? çek|çek)\b/i.test(
            lower,
          )
        ? "withdraw"
        : /\b(lend|supply|deposit|earn|borç ver|faize yatır|mevduat yatır|yatır)\b/i.test(
              lower,
            )
          ? "lend"
          : null;
  ...
  const swapVerb =
    /\b(swap|swapp?ing|convert|converting|takas|değiştir|cevir|çevir|buy|satın al|al|sell|sat)\b/i.exec(
      lower,
    );
  if (swapVerb && tokens.length >= 2 && amount) {
    ...
    if (/\b(buy|satın al|al)\b/i.test(lower)) {
      ...
      if (lower.startsWith("buy") || lower.startsWith("satın al")) {
      ...
      tokens.find(
        ({ symbol, index }) =>
          ...
          (index === inputToken.index ||
            extractAmountForToken(prompt, { symbol, index }) !== undefined),
      ) || inputToken;
    const explicitInputAmount = extractAmountForToken(prompt, inputToken);
    const swapAmount =
      explicitInputAmount || (amount === "MAX" ? amount : undefined);
    const outputToken = tokens.find(({ symbol }) => symbol === tokenOut);
    if (tokenIn !== tokenOut && swapAmount) {
      return deterministicIntent({
        ...
        minimumOutput: extractMinimumOutputForToken(prompt, outputToken),
...
export function parseDeterministicArcIntent(
  ...
  match =
    /^(?:I\s+want\s+to\s+)?(?:place|put|deposit|supply)\s+(\d+(?:[.,]\d+)?)\s+(?:native\s+)?(USDC|KLET)\s+(?:into|in|to)\s+(?:the\s+)?Kletia\s+lending\s+market\s+on\s+Arc\s+Testnet\s+as\s+collateral;\s*prepare\s+(?:a|the)\s+reviewable\s+route\s+before\s+wallet\s+approval$/iu.exec(
      prompt,
    );
  if (match) {
    return deterministicIntent({
  ...
  if (
    /^Withdraw my full Kletia Vault position, including available principal and interest, on Arc Testnet; simulate it before wallet approval$/i.test(
      prompt,
    )
  ) {
    return deterministicIntent({
  ...
  if (
    /^Claim all available rewards from Kletia Staking on Arc Testnet; simulate it before wallet approval$/i.test(
      prompt,
    )
  ) {
    return deterministicIntent({
  ...
  if (
    /^Claim my cooled-down unstaked native USDC from Kletia Staking on Arc Testnet; simulate it before wallet approval$/i.test(
      prompt,
    )
  ) {
    return deterministicIntent({
  ...
  if (prompt.startsWith(atomicPrefix) && prompt.endsWith(atomicSuffix)) {
    const payoutText = prompt.slice(atomicPrefix.length, -atomicSuffix.length);
    ...
    if (entries.length > 0 && entries.length <= 25 && entries.every(Boolean)) {
  ...
  if (
    /^Show my Arc portfolio and explain which Arc money routes are available without sending a transaction$/i.test(
      prompt,
    )
  ) {
    return deterministicIntent({
...
- "all", "max", "everything", "my holdings", "my balance", or possessive phrases such as "my Base USDC" must become amount "MAX" and amountSource "wallet_balance" on that workflow step.
- When a workflow step consumes the output of the immediately preceding swap or bridge, use amount "MAX" and amountSource "previous_output". Never reinterpret this as the wallet's entire destination balance.
...
- workflow: compile two or more explicitly ordered Base/Arbitrum actions into a staged plan. Preserve the user's order, assets and amounts in workflowSteps. Add a final borrow_capacity step when the user asks only how much could safely be borrowed after supply; do not create a borrow transaction. A real Base x402 GET purchase is represented as data_purchase and must retain url, method and maxPayment. A gas_acquire step must retain tokenIn USDC, tokenOut ETH, exact ETH amount, destinationChain arbitrum, and maxPayment as the maximum Base USDC spend. Never describe a cross-chain workflow as globally atomic.
...
User: "Use all my Base USDC to buy ETH, bridge the output to Arbitrum, lend it at the best supported live rate, then tell me how much USDC I can safely borrow"
...
export async function parseUserIntent(
  ...
  if (network === "arbitrum") {
    const readOnlyIntent = parseDeterministicArbitrumIntent(userPrompt);
    if (readOnlyIntent) {
      return readOnlyIntent.isComplete
        ? enforcePromptBoundIntent(readOnlyIntent, bindingText, network)
        : readOnlyIntent;
    }
  }
  ...
  if (
    network === "arc" &&
    /\[(?:AMOUNT|SLIPPAGE|MINIMUM_OUTPUT|RECIPIENT_ADDRESS|PUBLIC_REFERENCE)\]/i.test(
      userPrompt,
    )
  ) {
    return {
  ...
  if (network === "base") {
    const constraintFailure = baseExecutionConstraintFailure(bindingText);
  ...
  try {
    ...
    if (
      ...
      (!parsedJson.tokenIn ||
        !networkConfig.widgets.includes(
          String(parsedJson.tokenIn).toLowerCase(),
        ))
    ) {
      parsedJson = {
        ...parsedJson,
    ...
    if (
      ...
      if (!parsedJson.tokenIn || parsedJson.tokenIn.trim() === "") {
        return {
          ...
          message:
            parsedJson.message ||
            "Could you specify which .base.eth name you want to interact with buddy?",
          question: "",
          amount: "0",
          durationInDays: 0,
        };
    ...
    if (parsedJson.isComplete) {
      ...
      if (
        singleAssetActions.includes(parsedJson.action) &&
        !parsedJson.tokenIn &&
        parsedJson.tokenOut
      ) {
      ...
      if (
        ...
        parsedJson.borrowToken = normalizeAssetMention(parsedJson.borrowToken);
      }

      const amountText = String(parsedJson.amount ?? "0").trim();
      parsedJson.amount =
        amountText.toUpperCase() === "MAX" ? "MAX" : amountText;
    }
