...
export const BASE_TOKEN_DEPLOYMENT_MODE_ENV =
  'BASE_TOKEN_DEPLOYMENT_MODE';
...
export type BaseTokenDeploymentMode =
  | 'legacy_v1'
  | 'launch_v2';
...
export type BaseTokenDeploymentConfig =
  | LegacyTokenDeploymentConfig
  | LaunchFactoryV2TokenDeploymentConfig;
...
const PUBLIC_ERRORS = {
  ...
  BASE_TOKEN_DEPLOYMENT_MODE_INVALID: {
    message:
      'Base token deployment modu legacy_v1 veya launch_v2 olmalıdır.',
    statusCode: 500,
  },
  BASE_LAUNCH_FACTORY_V2_CONFIG_INVALID: {
    message:
      'Base Launch Factory V2 dağıtım kanıtı eksik veya doğrulanamadı.',
    statusCode: 503,
  },
  ...
  TOKEN_DEPLOYMENT_SIMULATION_FAILED: {
    message:
      'Token oluşturma işlemi canlı Base simülasyonunu geçemedi.',
    statusCode: 400,
  },
} as const;
...
export function parseBaseLaunchFactoryV2DeploymentEvidence(
  ...
  const encoded =
    environment[BASE_LAUNCH_FACTORY_V2_EVIDENCE_ENV]?.trim();
  ...
  const pendingTreasury = checkedAddress(
    record.pendingTreasury,
    true,
  );
  ...
  const evidence: BaseLaunchFactoryV2DeploymentEvidence = {
    ...
    observedAtBlock: checkedDecimal(
    ...
    ownerTimelockCodehash: checkedBytes32(
    ...
    treasurySafeCodehash: checkedBytes32(
      record.treasurySafeCodehash,
    ),
    pendingTreasury: ZERO_ADDRESS,
    factoryFeeCap: checkedDecimal(
      record.factoryFeeCap,
      false,
    ),
    maxTokenSupply: checkedDecimal(
      record.maxTokenSupply,
      false,
    ),
    maxNameBytes: checkedDecimal(
      record.maxNameBytes,
      false,
    ),
    maxSymbolBytes: checkedDecimal(
      record.maxSymbolBytes,
      false,
    ),
  };
  if (
...
export function resolveBaseTokenDeploymentConfig(
  ...
  const rawMode =
    environment[BASE_TOKEN_DEPLOYMENT_MODE_ENV]?.trim();
  if (!rawMode) {
  ...
  const deployment =
    parseBaseLaunchFactoryV2DeploymentEvidence(environment);
