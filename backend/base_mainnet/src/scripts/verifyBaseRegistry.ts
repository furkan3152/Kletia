...
import { discoverLiquidityPools } from '../dex/liquidityPools.js';
import { getLendingOpportunities } from '../lending/markets.js';
import {
    BASE_FEE_ROUTER,
...
async function main() {
    ...
    const v2RouterIdentityChecks = await mapBounded(
        ...
        async (router) => {
            ...
            const factoryHasCode = Boolean(
                factoryCode && factoryCode !== '0x',
            );
            const canonicalWeth = sameAddress(
                weth,
                BASE_TOKEN_REGISTRY.WETH.address,
            );
            if (!factoryHasCode) {
    ...
    const [aerodromeDefaultFactory, aerodromeFactoryRegistry] =
        await Promise.all([
            publicClient.readContract({
    ...
    if (!aerodromeFactoryApproved) {
        failures.push(
            `AERODROME_FACTORY_NOT_APPROVED:${ROUTERS.AERO_FACTORY}`,
        );
    }
    ...
    for (const { id, target, approved } of requiredAllowlist) {
        if (!approved) {
            failures.push(
                `FEE_ROUTER_REQUIRED_TARGET_NOT_APPROVED:${id}:${target}`,
            );
        }
    }
    for (const { id, target, approved } of forbiddenAllowlist) {
        if (approved) {
            failures.push(
                `FEE_ROUTER_FORBIDDEN_TARGET_APPROVED:${id}:${target}`,
            );
        }
    }

    const requiredTargetSet = new Set(
        BASE_FEE_ROUTER_REQUIRED_TARGETS.map(({ target }) =>
            target.toLowerCase(),
        ),
    );
    const historicalTargets = [
        ...approvalHistory.finalEventState.values(),
    ];
    ...
    const liquidityTargets = [
        ...new Map(
            liquidityPools.flatMap(({ router, factory, pool }) =>
                [router, factory, pool]).map((target) => [
                target.toLowerCase(),
                target,
            ]),
        ).values(),
    ...
    console.log(JSON.stringify({
        ...
        feeRouter: {
            ...
            state: {
                ...
                expectedFeeBps:
                    BASE_FEE_ROUTER_EXPECTED_FEE_BPS.toString(),
            ...
            staleTargets: forbiddenAllowlist.filter(({ id }) =>
                BASE_FEE_ROUTER_STALE_TARGETS.some(
                    (target) => target.id === id,
                ),
            ),
            ...
            note:
                'The release policy is exact: only requiredTargets may be true. Discovery, lending, vault, staking, stale and expansion targets must remain false.',
        },
        swapIdentity: {
        ...
        liveUsdcAeroLiquidity: {
            observedBlock:
                liquidityPools[0]?.observedBlock.toString() ?? null,
            pools: liquidityPools.map((pool) => ({
                ...
                unavailableSourceCount:
                    pool.unavailableSourceCount,
            })),
            note:
                'Factory-bound reserve snapshot only; no fee APR, future yield or impermanent-loss projection is inferred.',
        },
        failures,
    }, null, 2));
