...
import { KLETIA_INTENT_ROUTER_V2_ABI } from './baseIntentRouterV2Abis.js';
import { validateBaseIntentV2Runtime } from './baseIntentRouterV2Runtime.js';
import {
    BASE_SWAP_QUOTE_COLLECTION_POLICY,
    collectBaseSwapQuotes,
} from './baseSwapQuoteCollector.js';
import {
...
export interface BaseIntentV2SwapCoverage {
    ...
    readonly rankingMetric:
        'simulation_then_guaranteed_net_minimum';
    readonly noLegacyFallback: true;
}

export interface BaseIntentV2CompiledRoute
    extends BaseIntentV2SwapPlan {
...
function checkedPositiveAtomic(value: unknown): bigint {
    if (
        typeof value !== 'string' ||
        !DECIMAL_ATOMIC_PATTERN.test(value)
    ) {
...
function reviewedRouteFromQuote(
    ...
    const isUniV2 =
        value.quoteSource === SWAP_QUOTE_SOURCES.standardAmm;
    const isUniV3 =
        value.quoteSource === SWAP_QUOTE_SOURCES.v3Amm;
    if (!isUniV2 && !isUniV3) {
        return null;
    }
...
function assertQuoteAmountResolution(
    ...
    if (
        ...
        !DECIMAL_ATOMIC_PATTERN.test(
            value.observedBalanceAtomic,
        ) ||
    ...
    const expectedReserve = isNativeIn
        ? parseUnits('0.001', 18)
        : 0n;
...
BaseIntentV2SwapIntegrationDependencies {
    return {
...
export async function executeBaseIntentV2Swap(
    ...
    userAddress: string,
    configured: IntentV2ExecutionConfig,
    dependencies: BaseIntentV2SwapIntegrationDependencies =
    ...
    const freshDeployment =
        await dependencies.validateRuntime(configured);
    const executionConfig = normalizeExecutionConfig(
        configured,
        freshDeployment,
    );
    const observedTimestamp = checkedBlockTimestamp(
        await dependencies.blockTimestamp(
            freshDeployment.observedAtBlock,
        ),
    );

    const rawResult = await dependencies.collectQuotes(
        { ...intent },
        owner,
    );
    if (
    ...
    const amountIn = checkedPositiveAtomic(
        rawResult.amountInWei,
    );
    const isNativeIn = intent.tokenIn?.trim().toUpperCase() ===
        'ETH';
    const isNativeOut =
        intent.tokenOut?.trim().toUpperCase() === 'ETH';
    if (
    ...
    try {
        quotedTokenIn = getAddress(
            String(rawResult.tokenInAddress),
        );
        quotedTokenOut = getAddress(
            String(rawResult.tokenOutAddress),
        );
    } catch {
        return v2Error('BASE_INTENT_V2_NO_ELIGIBLE_ROUTE');
    }
    ...
    const deadline =
        observedTimestamp + V2_INTENT_TTL_SECONDS;
    const quoteExpiresAt =
        quoteExpiryFromDeadline(deadline);
    ...
    const compiled = reviewedQuotes.map(({ source, reviewed }) => {
        ...
        const routePath =
            ...
            source.routePath.trim().length > 0
                ? source.routePath.trim()
                : reviewedTokenPath(reviewed).join(' → ');
        const netMinimum = BigInt(
            plan.economics.netMinimumAmountOut,
        );
        return {
            source,
            plan,
    ...
    for (const result of simulationResults) {
        ...
        eligibleRoutes.push({
            ...
            quoteSource:
                candidate.plan.adapterKind ===
                    'uniswap_v3_swaprouter02'
                    ? 'v3_amm'
                    : 'standard_amm',
            network: 'base',
            approvals: candidate.plan.approvals.map(
                (approval) => ({
                    ...approval,
    ...
    const coverage: BaseIntentV2SwapCoverage = {
        ...
        observedAtBlock:
            freshDeployment.observedAtBlock.toString(),
        ...
        unsupportedQuoteCount:
            rawResult.allRoutes.length - reviewedQuotes.length,
    ...
    return {
        ...
        protocolExclusionEvidence:
            rawResult.protocolExclusionEvidence,
        rankingEvidence: {
            policyVersion:
