...
function safeSession(rawSessionId: unknown): BaseX402BuyerSession {
  if (
...
function parsePaymentSignatureHeader(
  ...
  if (
    ...
    validBefore >
      now + BigInt(session.accepted.maxTimeoutSeconds) + CLOCK_SKEW_SECONDS
  ) {
    throw new BaseX402BuyerRouteError(
...
function validateSettlement(header: string, session: BaseX402BuyerSession) {
  let decoded: unknown;
  try {
...
function safeContentType(upstream: BaseX402BuyerUpstreamResponse): string {
...
function setPrivateResponseHeaders(res: Response, requestId?: string): void {
  ...
  try {
    ...
    try {
      ...
      const session: BaseX402BuyerSession = {
        ...
        paymentRequired: prepared.paymentRequired as Readonly<
...
router.get("/session/:sessionId/status", (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      ...sessionIdentity(session),
      paymentState: session.state,
      retryable: session.state === "prepared" && session.expiresAt > Date.now(),
      expiresAt: new Date(session.expiresAt).toISOString(),
...
router.get("/session/:sessionId", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const settlement = validateSettlement(candidatePaymentResponse, session);
