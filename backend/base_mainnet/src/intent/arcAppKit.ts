...
export const ARC_APP_KIT_TOKENS = [
  'USDC',
  'EURC',
  'cirBTC',
] as const;
...
function slippageBps(value: unknown): number {
  const rawPercent = String(value ?? '1').replace('%', '').trim();
...
export function buildArcAppKitPlan(
  intent: ParsedIntent,
  requestId: string,
) {
  ...
  if (action === 'stable_swap') {
    ...
    expectedOutput =
      `${amount} ${tokenIn} için canlı App Kit ${tokenOut} tahmini`;
    ...
    expectedOutput =
      `${amount} USDC → ${executionPlan.destinationChain} / ${recipient}`;
  } else {
    throw new ArcAppKitPlanError(
...
export function isArcAppKitResultBinding(
  ...
  const operation =
    ACTION_OPERATION_BINDING[
      action as keyof typeof ACTION_OPERATION_BINDING
    ];
  const plan =
  ...
  try {
    ...
    if (plan.operation === 'bridge') {
      if (
        ...
        (plan.transferSpeed !== 'FAST' &&
          plan.transferSpeed !== 'SLOW')
      ) {
        return false;
      }
