...
export const AlloraDashboard: React.FC<AlloraDashboardProps> = ({ isDarkMode }) => {
      ...
      try {
        ...
        const payload =
          typeof json === 'object' && json !== null && !Array.isArray(json)
            ...
            : null;
        const predictions = Array.isArray(payload?.data)
          ? payload.data
          : null;
        const hasExpectedAssets =
          predictions?.length === ALLORA_ASSETS.length &&
          new Set(predictions.map((item) =>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center border-b-4 border-current pb-4">
        <div>
          <h1 className={`text-4xl font-black uppercase tracking-tighter ${headerText}`}>
            Allora <span className={isDarkMode ? 'text-gray-100' : 'text-gray-900'}>Fiyat Gözlemi</span>
          </h1>
            ...
            return (
              ...
              >
                <div className="flex justify-between items-center border-b-2 border-dashed border-gray-500 pb-2">
                  ...
                  <span className={`text-sm font-bold uppercase px-2 py-1 border-2 ${directionClass}`}>
                    {item.direction}
                  </span>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <span className={`text-lg font-bold mb-1 ${diffColor}`}>
                      {isPositive ? '▲' : diffPercent < 0 ? '▼' : '•'} {item.predictedDeltaPercent}%
                    </span>
                  </div>
                </div>
