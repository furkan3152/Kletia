...
type Availability = 'available' | 'partial' | 'unavailable' | 'not_configured' | 'not_needed';
...
function report(
    ...
    observedAt: string,
    method: string,
    details: Omit<SourceReport, 'source' | 'status' | 'partial' | 'observedAt' | 'method'> = {},
): SourceReport {
    return {
        source,
        status,
        partial: status === 'partial' || status === 'unavailable' || status === 'not_configured',
        observedAt,
        method,
        ...details,
    };
...
async function fetchJson(url: string, init?: RequestInit): Promise<ReadResult<any>> {
    try {
        const response = await fetch(url, {
            ...init,
            signal: AbortSignal.timeout(12_000),
        });
        if (response.ok === false) return { ok: false, reason: `http_${response.status}` };
...
function combineAvailability(reports: SourceReport[]): 'complete' | 'partial' | 'unavailable' {
    ...
    const incomplete = relevant.some((item) =>
        item.status === 'partial' || item.status === 'unavailable' || item.status === 'not_configured'
    );
...
async function fetchPrices(addresses: string[], observedAt: string): Promise<{
    ...
    if (uniqueAddresses.length === 0) {
        return {
            quotes,
            source: report('dexscreener', 'not_needed', observedAt, 'latest/dex/tokens', {
                requested: 0,
                resolved: 0,
            }),
        };
    ...
    for (let index = 0; index < uniqueAddresses.length; index += 30) {
        ...
        const result = await fetchJson(`https://api.dexscreener.com/latest/dex/tokens/${chunk.join(',')}`);
        if (!result.ok) {
            failedChunks += 1;
            continue;
        }
        ...
        for (const pair of pairs) {
            ...
            const address = typeof pair?.baseToken?.address === 'string'
            ...
            const normalizedLiquidity = Number.isFinite(liquidityUsd) ? liquidityUsd : 0;
    ...
    const resolved = uniqueAddresses.filter((address) => quotes.has(address)).length;
    const status: Availability = resolved === uniqueAddresses.length && failedChunks === 0
        ? 'available'
        : resolved > 0
            ? 'partial'
            : 'unavailable';
...
function summarizeAssets(assets: Array<WalletAsset | LiquidStakingAsset>, discoveryComplete: boolean) {
    const priced = assets.filter((asset) => typeof asset.usdValue === 'number');
    const knownValue = priced.reduce((sum, asset) => sum + (asset.usdValue ?? 0), 0);
...
export async function getPortfolio(userAddress: string) {
    ...
    try {
        ...
        sources.nativeBalance = report('base_rpc', 'available', observedAt, 'eth_getBalance', {
        ...
        sources.nativeBalance = report('base_rpc', 'unavailable', observedAt, 'eth_getBalance', {
    ...
    if (!alchemyRpcUrl) {
        sources.erc20Balances = report('alchemy', 'not_configured', observedAt, 'alchemy_getTokenBalances', {
            reason: 'ALCHEMY_API_KEY_missing',
            scope: 'All positive ERC-20 wallet balances',
        });
        sources.erc20Metadata = report('base_rpc', 'not_needed', observedAt, 'erc20_metadata', {
        ...
        const tokenBalances = result.ok && Array.isArray(result.value?.result?.tokenBalances)
            ? result.value.result.tokenBalances
            : undefined;

        if (!tokenBalances) {
            sources.erc20Balances = report('alchemy', 'unavailable', observedAt, 'alchemy_getTokenBalances', {
            ...
            const positiveBalances: Array<{ address: `0x${string}`; balance: bigint }> = [];
            ...
            sources.erc20Balances = report('alchemy', invalidBalanceRecords > 0 ? 'partial' : 'available', observedAt, 'alchemy_getTokenBalances', {
                records: discoveredTokenCount,
                failures: invalidBalanceRecords,
                reason: invalidBalanceRecords > 0 ? 'invalid_provider_balance_record' : undefined,
                scope: 'All positive ERC-20 wallet balances',
            });

            await Promise.all(positiveBalances.map(async ({ address, balance }) => {
                const [symbolResult, nameResult, decimalsResult] = await Promise.all([
                    readContract<string>({ address, abi: erc20Abi, functionName: 'symbol' }),
                    readContract<string>({ address, abi: erc20Abi, functionName: 'name' }),
                    readContract<number>({ address, abi: erc20Abi, functionName: 'decimals' }),
                ]);

                const decimals = decimalsResult.ok ? Number(decimalsResult.value) : undefined;
                const decimalsValid = decimals !== undefined
                    && Number.isInteger(decimals)
                    && decimals >= 0
                    && decimals <= 255;
                const metadataStatus = decimalsValid && symbolResult.ok && nameResult.ok
                    ? 'available'
                    : decimalsValid
                        ? 'partial'
                        : 'unavailable';
                ...
                const exactAmount = decimalsValid ? formatUnits(balance, decimals) : undefined;
                const lowerAddress = address.toLowerCase();

                wallet.push({
                    symbol: symbolResult.ok && symbolResult.value
                        ? symbolResult.value
                        : addressLabel,
                    ...
                    balanceStatus: 'available',
                    metadataStatus,
                    _quantity: exactAmount === undefined ? undefined : Number(exactAmount),
                    _priceAddress: lowerAddress,
                });
                tokenAddressesToPrice.add(lowerAddress);
            }));

            const metadataStatus: Availability = discoveredTokenCount === 0
                ? 'not_needed'
                : metadataFailures === 0
                    ? 'available'
                    : metadataFailures < discoveredTokenCount
                        ? 'partial'
                        : 'unavailable';
            sources.erc20Metadata = report('base_rpc', metadataStatus, observedAt, 'symbol/name/decimals', {
                requested: discoveredTokenCount,
                resolved: Math.max(0, discoveredTokenCount - metadataFailures),
                failures: metadataFailures,
                reason: metadataFailures > 0 ? 'one_or_more_metadata_reads_failed' : undefined,
            });
        }
    }
    ...
    sources.liquidStaking = report(
        'base_rpc',
        lstReadFailures === 0 ? 'available' : lstReadFailures < lstTokens.length ? 'partial' : 'unavailable',
        observedAt,
        'balanceOf',
        {
            ...
            reason: lstReadFailures > 0 ? 'one_or_more_lst_balance_reads_failed' : undefined,
        },
    );

    const baseNames: { tokenId: string; name?: string; index: number }[] = [];
    if (!alchemyApiKey) {
        sources.basenames = report('alchemy', 'not_configured', observedAt, 'nft/v3/getNFTsForOwner', {
            reason: 'ALCHEMY_API_KEY_missing',
        ...
        const ownedNfts = result.ok && Array.isArray(result.value?.ownedNfts)
        ...
        if (ownedNfts) {
            for (const nft of ownedNfts) {
                try {
                    baseNames.push({
                        ...
                        name: typeof nft.name === 'string' && nft.name.trim() ? nft.name : undefined,
            ...
            sources.basenames = report('alchemy', invalidRecords > 0 ? 'partial' : 'available', observedAt, 'nft/v3/getNFTsForOwner', {
            ...
            sources.basenames = report('alchemy', 'unavailable', observedAt, 'nft/v3/getNFTsForOwner', {
                reason: result.ok ? 'unexpected_provider_response' : result.reason,
                scope: 'Basenames owned by the address',
            });
        }
    ...
    const aaveResult = await readContract<readonly [bigint, bigint, bigint, bigint, bigint, bigint]>({
        address: AAVE_POOL,
    ...
    if (!aaveResult.ok) {
        sources.aave = report('base_rpc', 'unavailable', observedAt, 'getUserAccountData', {
        ...
        const [collateralRaw, debtRaw, availableBorrowRaw, , , healthFactorRaw] = aaveResult.value;
        ...
        if (collateralRaw > 0n || debtRaw > 0n) {
            ...
            const healthFactor = debtFree ? undefined : Number(formatUnits(healthFactorRaw, 18));
            defiPositions.aave = {
                ...
                status: debtFree ? 'SAFE' : (healthFactor ?? 0) > 1.5 ? 'HEALTHY' : 'WARNING',
            };
        }
        sources.aave = report('base_rpc', 'available', observedAt, 'getUserAccountData', {
            records: collateralRaw > 0n || debtRaw > 0n ? 1 : 0,
        });
    }

    const moonwellMarkets = MOONWELL_BASE.markets.map(
        ({ token, market }) => ({
    ...
    if (Object.keys(moonwellData).length > 0) defiPositions.moonwell = moonwellData;
    sources.moonwell = report(
        'base_rpc',
        moonwellFailures === 0 ? 'available' : moonwellResolvedReads > 0 ? 'partial' : 'unavailable',
        observedAt,
        'borrowBalanceStored/balanceOfUnderlying',
        {
            ...
            reason: moonwellFailures > 0 ? 'one_or_more_market_reads_failed' : undefined,
            scope:
    ...
    const compoundData: Record<
        string,
        { supplied: string; debt: string }
    > = {};
    ...
    for (const {
        ...
        compoundFailures +=
            Number(!borrowResult.ok) + Number(!supplyResult.ok);
    ...
    const compoundRequestedReads =
        COMPOUND_V3_BASE.markets.length * 2;
    sources.compound = report(
        ...
        'borrowBalanceOf/balanceOf',
        {
            ...
            reason: compoundFailures > 0 ? 'one_or_more_comet_reads_failed' : undefined,
            scope:
                `${COMPOUND_V3_BASE.markets.length} registry-bound Base Comets`,
        },
    );
    ...
    if (!veBalanceResult.ok) {
        sources.aerodrome = report('base_rpc', 'unavailable', observedAt, 'veAERO enumerable reads', {
            reason: veBalanceResult.reason,
        });
    } else if (veBalanceResult.value === 0n) {
        sources.aerodrome = report('base_rpc', 'available', observedAt, 'veAERO enumerable reads', {
            records: 0,
        });
    } else {
        const scanLimit = 25n;
        const countToScan = Number(veBalanceResult.value > scanLimit ? scanLimit : veBalanceResult.value);
        let lockedTotal = 0n;
        ...
        for (let index = 0; index < countToScan; index += 1) {
            ...
            if (lockedResult.value[1] > latestUnlock) latestUnlock = lockedResult.value[1];
        }
        ...
        const status: Availability = lockIds.length === 0
            ? 'unavailable'
            : hitLimit || lockReadFailures > 0
                ? 'partial'
                : 'available';
        sources.aerodrome = report('base_rpc', status, observedAt, 'veAERO enumerable reads', {
    ...
    if (!alchemyRpcUrl) {
        sources.recentTransactions = report('alchemy', 'not_configured', observedAt, 'alchemy_getAssetTransfers', {
            reason: 'ALCHEMY_API_KEY_missing',
        ...
        const transfers = result.ok && Array.isArray(result.value?.result?.transfers)
        ...
        if (transfers) {
            ...
            for (const transaction of transfers) {
                ...
                const numericValue = transaction.value === null || transaction.value === undefined
                    ? undefined
                    : Number(transaction.value);
                const amount = numericValue !== undefined && Number.isFinite(numericValue)
                    ? numericValue.toFixed(6)
                    : 'Amount unavailable';
                recentTransactions.push({
                    hash: transaction.hash,
                    from: transaction.from,
                    to: typeof transaction.to === 'string' ? transaction.to : 'Contract Creation',
                    value: `${amount}${transaction.asset ? ` ${transaction.asset}` : ''}`,
                    type: transaction.category === 'erc20' ? 'Token Transfer' : 'ETH Transfer',
                });
            }
            sources.recentTransactions = report('alchemy', invalidRecords > 0 ? 'partial' : 'available', observedAt, 'alchemy_getAssetTransfers', {
            ...
            sources.recentTransactions = report('alchemy', 'unavailable', observedAt, 'alchemy_getAssetTransfers', {
                reason: result.ok ? 'unexpected_provider_response' : result.reason,
                scope: 'Latest 20 outgoing external/ERC-20 transfers',
            });
        }
    ...
    for (const asset of wallet) {
        if (asset._priceAddress) applyPrice(asset, asset._priceAddress, priceResult.quotes);
    }
    ...
    const deduplicatedWallet = wallet.filter((asset) =>
        !asset.address || !independentlyReadLstAddresses.has(asset.address.toLowerCase())
    );
    ...
    for (const asset of deduplicatedWallet) {
        if (defiKeywords.test(asset.name ?? '') || defiKeywords.test(asset.symbol)) {
    ...
    const byKnownValue = (left: WalletAsset, right: WalletAsset) => {
        const leftValue = typeof left.usdValue === 'number' ? left.usdValue : Number.NEGATIVE_INFINITY;
        const rightValue = typeof right.usdValue === 'number' ? right.usdValue : Number.NEGATIVE_INFINITY;
    ...
    const regularWalletSummary = summarizeAssets(finalWallet, walletDiscovery === 'complete');
    const defiTokenSummary = summarizeAssets(defiTokens, walletDiscovery === 'complete');
    ...
    const pricedAssets = allValuedAssets.filter((asset) => typeof asset.usdValue === 'number');
    ...
    const unpricedAssets = allValuedAssets
        ...
        .map((asset) => ({
            ...
            reason: asset._quantity === undefined ? 'amount_metadata_unavailable' : 'price_quote_unavailable',
        }));
    const assetDiscoveryComplete =
        walletDiscovery === 'complete' && sources.liquidStaking.status === 'available';
        ...
        assetDiscoveryComplete && unpricedAssets.length === 0
            ...
            : pricedAssets.length > 0
                ...
                : allValuedAssets.length === 0 && assetDiscoveryComplete
                    ? 'complete'
                    : 'unavailable';
    const totalFormatted = valuationStatus === 'complete'
    ...
    const integrity = {
        ...
        valuation: {
            ...
            knownPricedValueUSD: pricedAssets.length > 0 || valuationStatus === 'complete'
                ? knownTotal.toFixed(2)
                : null,
            pricedAssetCount: pricedAssets.length,
            unpricedAssetCount: unpricedAssets.length,
            unpricedAssets,
            scope: 'Wallet and LST token balances only; NFTs and protocol positions are excluded from the USD total.',
    ...
    const warning = unavailableSources.length > 0
    ...
    return {
        ...
        message: `**💼 Kletia Base portfolio scan processed.**\n`
