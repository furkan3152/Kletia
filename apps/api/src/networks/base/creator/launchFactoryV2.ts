...
const STRICT_TOKEN_SUPPLY_PATTERN = /^(?:0|[1-9]\d*)(?:\.\d{1,18})?$/u;
const CONTROL_OR_FORMAT_PATTERN = /\p{C}/u;
const UNICODE_SEPARATOR_PATTERN = /\p{Z}/u;
const ZERO_CODE = "0x";

export type LaunchSaltSource = "explicit_launch_id" | "canonical_parameters";

export interface LaunchFactoryV2Evidence {
  readonly policyVersion: typeof KLETIA_LAUNCH_FACTORY_V2_POLICY_VERSION;
...
function runtimeError(): never {
  throw new BaseTokenLaunchError("BASE_LAUNCH_FACTORY_V2_RUNTIME_INVALID");
}
...
function validateMetadata(
  ...
  if (encoded.length === 0 || BigInt(encoded.length) > maximumBytes) {
    inputError();
  }
  if (
    ...
    return {
      userSalt: keccak256(
        encodeAbiParameters(
          ...
          [KLETIA_LAUNCH_USER_SALT_DOMAIN, EXPLICIT_LAUNCH_ID_KIND, launchId],
        ),
      ),
...
export async function validateLaunchFactoryV2Runtime(
  ...
  try {
    ...
    const pendingTreasury = checkedRuntimeAddress(pendingTreasuryResult);
    const deploymentFee = checkedRuntimeUint(deploymentFeeResult);
    const factoryFeeCap = checkedRuntimeUint(factoryFeeCapResult);
...
export async function buildLaunchFactoryV2TokenPlan(input: {
  ...
  if (input.requestedRecipient !== undefined) {
    try {
      if (
        !sameAddress(getAddress(String(input.requestedRecipient)), recipient)
      ) {
        inputError();
      }
    } catch (error) {
  ...
  const name = validateMetadata(input.name, runtime.maxNameBytes, true);
  const symbol = validateMetadata(input.symbol, runtime.maxSymbolBytes, false);
  ...
  try {
    ...
    if (!sameAddress(existingToken, ZERO_ADDRESS)) {
      throw new BaseTokenLaunchError("TOKEN_LAUNCH_SALT_ALREADY_USED");
    }
  ...
  try {
    ...
    if (
      ...
      !sameAddress(getAddress(simulation.result), predictedAddress)
    ) {
      throw new Error("SIMULATION_RESULT_MISMATCH");
    }
  } catch {
    throw new BaseTokenLaunchError("TOKEN_DEPLOYMENT_SIMULATION_FAILED");
  }
