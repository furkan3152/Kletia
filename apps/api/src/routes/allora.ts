...
function extractAlloraPrice(payload: unknown): number {
  ...
  const inference = asRecord(data?.inference_data ?? root?.inference_data);
...
async function getObservation(
  ...
  const deltaPercent = ((predictedPrice - currentPrice) / currentPrice) * 100;
...
router.post("/multi-prediction", async (req, res) => {
  ...
  const timeframe =
    rawTimeframe === undefined ? "5m" : parseTimeframe(rawTimeframe);
  if (!timeframe) {
    return res.status(400).json({
      success: false,
      error: "timeframe must be 5m or 8h.",
