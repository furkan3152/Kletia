...
function assertUsdcAmount(value: unknown, field: string): asserts value is string {
...
export async function readArcUnifiedUsdcBalance(
  ...
  if (!supportedTestnetChains.has('Arc_Testnet')) {
    throw new Error('Kurulu Circle App Kit Arc Testnet Gateway desteği sunmuyor.');
  }
  ...
  const accounts = raw.breakdown.map((entry): ArcUnifiedBalanceAccount => {
    ...
    const chains = entry.breakdown.map((chain): ArcUnifiedBalanceChain => {
      ...
      assertUsdcAmount(chain.confirmedBalance, `${chainName} doğrulanmış bakiye`);
