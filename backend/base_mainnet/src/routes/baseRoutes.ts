...
} from '../intent/baseX402AttestationRegistry.js';
...
router.get('/x402/attestations/status', async (_req, res) => {
  try {
    ...
    return res.status(known ? error.statusCode : 503).json({
      ...
      code: known
        ? error.code
        : 'X402_ATTESTATION_REGISTRY_UNAVAILABLE',
      message:
...
router.get('/x402/attestations/verify', (_req, res) => {
  ...
  return res.status(405).json({
    ...
    message:
      'x402 attestation claim proofs must be submitted as POST JSON.',
    network: 'base',
    chainId: NETWORKS.base.chainId,
  });
});
