...
import { Loader2, ShieldCheck, ShieldAlert, Search, AlertTriangle, Shield } from 'lucide-react';
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-[#0F172A] border-[3px] border-[#1A1A1A] dark:border-[#4B5563] shadow-[4px_4px_0_#1A1A1A] dark:shadow-[4px_4px_0_#475569] p-6 text-center">
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-[#1A1A1A] dark:text-white">Due Diligence Hub</h1>
          </div>
          <p className="text-sm md:text-base font-bold text-gray-600 dark:text-gray-400">
            Powered by Webacy (DD.xyz). Enter smart contract or wallet address to scan, view risk analysis instantly.
          </p>
        </div>
        ...
        <form onSubmit={handleScan} className="flex flex-col md:flex-row gap-3 w-full">
          ...
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" strokeWidth={3} /> : <Search className="w-5 h-5" strokeWidth={3} />}
            TARA
          </button>
        </form>
        ...
        {result && (
          <div className="bg-white dark:bg-[#0F172A] border-[3px] border-[#1A1A1A] dark:border-[#4B5563] shadow-[4px_4px_0_#1A1A1A] dark:shadow-[4px_4px_0_#475569] flex flex-col overflow-hidden">
            <div className={`p-6 border-b-[3px] border-[#1A1A1A] dark:border-[#4B5563] flex flex-col md:flex-row items-center justify-between gap-4 ${
              result.decision === 'blocked' ? 'bg-red-500 text-white' : result.riskScore > 20 ? 'bg-yellow-400 text-black' : 'bg-[#00d66f] text-black'
            }`}>
              <div className="flex items-center gap-3">
                {result.decision === 'blocked' ? <ShieldAlert className="w-10 h-10" strokeWidth={3} /> : <ShieldCheck className="w-10 h-10" strokeWidth={3} />}
                <div className="flex flex-col">
                  <span className="text-xl font-black uppercase tracking-wider">
                    WEBACY: {result.riskLevel} RISK
                  </span>
                  <span className="text-sm font-bold opacity-80 uppercase tracking-widest">
                    {result.isContract ? 'Smart Contract' : 'Wallet (EOA)'} · {result.decision}
                  </span>
                </div>
              </div>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              {result.tags && result.tags.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {result.tags.map((tag: string, idx: number) => (
                     <span key={idx} className="bg-[#1A1A1A] dark:bg-slate-700 text-white text-xs font-bold uppercase tracking-wider px-3 py-1.5 border-[2px] border-transparent">
                       {tag}
                     </span>
                  ))}
                </div>
                ...
                <div className="text-gray-500 dark:text-gray-400 font-bold italic">
                  Provider risk etiketi bildirmedi; bu, adresin risksiz olduğuna dair garanti değildir.
                </div>
              )}
              ...
              {result.decision === 'blocked' && (
                <div className="mt-4 bg-red-100 dark:bg-red-900/30 p-4 border-[3px] border-red-500 font-bold text-red-700 dark:text-red-400">
                  ⚠️ WARNING: Kletia Autonomous Agent and Kletia Firewall will not allow you to make any transaction with this address.
                </div>
              )}
            </div>
          </div>
