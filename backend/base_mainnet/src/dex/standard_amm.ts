...
export async function getUniswapAndV2Routes(
    ...
    userAddress: string, deadline: bigint, decimalsOut: number, slippageBps: number = 100
) {
    const isNativeOut = tokenOutSymbol === "ETH"; 
    const rawRoutes: any[] = [];

    const bases = [TOKENS["WETH"], TOKENS["USDC"], TOKENS["AERO"], TOKENS["DAI"], TOKENS["CBBTC"]];
    ...
    await checkRouter(ROUTERS.PANCAKE_V2, "PancakeSwap");
    await checkRouter(ROUTERS.SUSHI_V2, "SushiSwap");
    await checkRouter(ROUTERS.BASESWAP, "BaseSwap");
    await checkRouter(ROUTERS.SWAPBASED, "SwapBased");
