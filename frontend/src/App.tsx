...
export default function App() {
  ...
  const { isDarkMode, isArcMode, messages, addMessage, updateMessage, addTerminalLog, initSocket } = useAppStore();
  ...
  useEffect(() => {
    if (isArcMode) {
      ...
      document.documentElement.classList.remove('arc-mode');
      if (activeTab === 'arc' || activeTab === 'lending') {
        setActiveTab('chat');
      }
    }
  }, [isArcMode, activeTab]);
  ...
  const handleSend = async () => {
    ...
    try {
      const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://127.0.0.1:3001';
      const endpoint = (isAgentMode && !isArcMode) ? '/api/agent/chat' : '/api/intent';

      const response = await fetch(`${BACKEND_URL}${endpoint}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: userText, userAddress: address, msgId: kletiaMsgId, isArcMode })
      });

      if (isAgentMode && !isArcMode) {
        if (!response.ok) {
           const errorData = await response.json().catch(() => ({}));
           updateMessage(kletiaMsgId, { isLoading: false, text: `❌ Agent Error: ${errorData.message || 'Bilinmeyen hata'}` });
           return;
        }
        ...
        let agentText = "🤖 Agent is starting operations...\n";
        updateMessage(kletiaMsgId, { isLoading: true, text: agentText, terminalLogs: ["Switched to Agent mode..."] });
              ...
              for (const line of lines) {
                 if (line.startsWith('event: ')) {
                    ...
                    try {
                        ...
                        if (eventType === 'log') {
                            addTerminalLog(kletiaMsgId, `[LOG] ${data}`);
                        } else if (eventType === 'agent') {
                            agentText += `\n**Ajan:** ${data}\n`;
                            addTerminalLog(kletiaMsgId, `🧠 Agent is thinking/replying...`);
                            updateMessage(kletiaMsgId, { text: agentText });
                        } else if (eventType === 'tools') {
                            // Trim tool data if it's too long
                            const toolStr = typeof data === 'string' ? data : JSON.stringify(data);
                            const displayStr = toolStr.length > 200 ? toolStr.substring(0, 200) + '...' : toolStr;
                            addTerminalLog(kletiaMsgId, `🛠️ Tool Executed: ${displayStr}`);
                        } else if (eventType === 'error') {
                            agentText += `\n❌ **Error:** ${data}\n`;
                            addTerminalLog(kletiaMsgId, `❌ HATA: ${data}`);
                            updateMessage(kletiaMsgId, { text: agentText, isLoading: false });
                        } else if (eventType === 'done') {
                            addTerminalLog(kletiaMsgId, `✅ Task completed.`);
                            updateMessage(kletiaMsgId, { isLoading: false });
                        }
                    } catch (e) {}
                 }
              }
  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-[#EFEFEF] dark:bg-[#0B1120] text-[#1A1A1A] dark:text-gray-100 font-sans antialiased overflow-hidden transition-colors duration-300">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none select-none">
        ...
        <div className="hidden md:block absolute right-[-20px] bottom-[20%] text-[160px] font-black text-black/[0.03] dark:text-white/[0.02] rotate-12 tracking-widest">{isArcMode ? 'KLET' : 'OMNI'}</div>
