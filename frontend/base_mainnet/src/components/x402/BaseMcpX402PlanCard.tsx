...
} from '../../x402/baseX402RelayBoundary';
  ...
  const approveAndPay = async () => {
    ...
    try {
      ...
      if (getAddress(address) !== expectedAccount) {
        throw new Error('Aktif cüzdan, bu x402 planındaki cüzdanla eşleşmiyor.');
      }
      if (
        ...
        throw new Error('x402 challenge kanıtı eskimiş veya planla eşleşmiyor.');
      }
      ...
      const buyer = await import('../../x402/baseX402Buyer');
      ...
      const paidResponse = await requestWithTimeout(relayUrl, {
        headers: {
          ...commonHeaders,
      ...
      if (isWalletRejection(error)) {
        ...
        setPaymentMessage('Cüzdan onayı kullanıcı tarafından iptal edildi; ödeme yapılmadı.');
