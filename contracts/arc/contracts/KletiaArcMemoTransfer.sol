...
contract KletiaArcMemoTransfer is ERC2771Context {
    ...
    modifier onlyOwner() {
        require(
            _msgSender() == owner,
            "KletiaArcMemoTransfer: caller is not the owner"
        );
        _;
    }
    ...
    function transferWithMemo(
        ...
        require(
            to != _msgSender(),
            "KletiaArcMemoTransfer: cannot transfer to self"
        );
        require(amount > 0, "KletiaArcMemoTransfer: amount must be > 0");
        require(
            bytes(memo).length > 0,
            "KletiaArcMemoTransfer: memo cannot be empty"
        );
        require(
            bytes(memo).length <= 256,
            "KletiaArcMemoTransfer: memo too long"
        );
    ...
    function getTransfer(
        ...
        require(
            transferId < nextTransferId,
            "KletiaArcMemoTransfer: transfer does not exist"
        );
    ...
    function getSentTransferIds(
        address addr
    ) external view returns (uint256[] memory) {
        return _sentTransfers[addr];
    }

    function getReceivedTransferIds(
        address addr
    ) external view returns (uint256[] memory) {
    ...
    function receivedTransferCount(
        address addr
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(
            newOwner != address(0),
            "KletiaArcMemoTransfer: invalid new owner"
        );
