...
export type SwapSimulationStatus =
    | 'passed'
...
export function rankSwapRoutes<T extends SwapRouteForRanking>(
    ...
    return [...routes].sort((left, right) => {
        const leftSimulationRank =
            left.simulationStatus === 'passed' ? 0 : 1;
        const rightSimulationRank =
            right.simulationStatus === 'passed' ? 0 : 1;
...
export function summarizeQuoteCoverage(
    ...
    const sources = inputs.map(({ source, result }): QuoteSourceCoverage => {
        ...
        return {
            ...
            ...(diagnostics?.protocols
                ? { protocols: diagnostics.protocols }
                : {}),
        };
    });
    ...
    return {
        ...
        sourceWithRoutesCount: sources.filter(
...
export function buildSwapRankingEvidence<T extends SwapRouteForRanking>(
    ...
    return {
        ...
        stage,
        primaryMetric:
            'simulation_evidence_then_quoted_amount_out' as const,
