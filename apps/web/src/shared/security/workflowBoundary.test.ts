...
const plan = {
  ...
  currentStepIndex: 0,
  steps: [
    { id: "step-1", order: 1, action: "bridge", network: "base" as const, chainId: 8453 as const, amount: "10", tokenIn: "USDC", destinationChain: "arbitrum", dependsOn: [], status: "awaiting_signature" as const, expectedOutputAtomic: "9900000" },
...
describe("WorkflowPlanV1 boundary", () => {
  ...
  it("accepts a terminal read-only borrow result and rejects fake or non-terminal results", () => {
    const capacityPlan = structuredClone(plan) as WorkflowPlanV1;
    capacityPlan.currentStepIndex = 1;
    capacityPlan.steps[1] = {
      ...capacityPlan.steps[1],
    ...
    expect(isWorkflowPlanV1(capacityPlan, { requestId, userAddress: owner, nowMs: now })).toBe(true);
    capacityPlan.steps[1].readResult!.mockData = true as false;
    expect(isWorkflowPlanV1(capacityPlan, { requestId, userAddress: owner, nowMs: now })).toBe(false);
  });
});
