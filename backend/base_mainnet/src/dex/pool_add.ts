...
export async function buildAddLiquidityRoutes(
    ...
    if (secondaryAmountStr !== undefined) {
        const normalizedSecondaryAmount =
            secondaryAmountStr.trim();
        if (
            !/^(?:\d+\.?\d*|\.\d+)$/.test(
                normalizedSecondaryAmount,
            ) ||
    ...
    if (requestedAmountA > availableA) {
        throw Object.assign(
            new Error(
                `INSUFFICIENT_FUNDS: Not enough ${tokenASymbol} in wallet.`,
            ),
    ...
    const routes = pools.flatMap((pool) => {
        let amountA = maxRequested
            ...
            : requestedAmountA;
        let amountB = amountA > 0n
            ? ceilDiv(amountA * pool.reserveB, pool.reserveA)
            : 0n;
        if (
            maxRequested &&
            amountB > routeAvailableB &&
            amountA > 0n
        ) {
            amountA -= 1n;
            amountB = amountA > 0n
        ...
        return [{
            ...
            primaryTokenAddress:
                assetA.isNative ? undefined : assetA.address,
            primaryAmountInWei:
                assetA.isNative ? undefined : amountA.toString(),
            secondaryTokenAddress:
                assetB.isNative ? undefined : assetB.address,
            secondaryAmountInWei:
                assetB.isNative ? undefined : amountB.toString(),
            approvals,
            ...
            poolEvidence: {
                ...
                secondaryAmountCapAtomic:
                    secondaryAmountCap?.toString(),
                ...
                limitation:
                    `LP share output and future fee APY are established only by execution; impermanent-loss risk is not projected. Minimum amounts use ${slippageBps} bps slippage.`,
            },
        }];
    });
