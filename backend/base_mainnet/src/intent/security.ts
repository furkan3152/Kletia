...
import { BASE_TOKEN_REGISTRY } from '../config/baseProtocols.js';
...
export async function checkTokenSecurity(
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Token security provider returned HTTP ${response.status}.`);
        }
        ...
        const registeredTokenSignalException =
            policy === 'registered' &&
            REGISTERED_TOKEN_ADDRESSES.has(
        ...
        if (
            ...
            throw new Error('Token security provider returned incomplete risk signals.');
        }
...
export async function xRaySimulate(
    ...
    try {
        ...
        if (needsApproval) {
            console.log(`⚠️ [X-RAY] ${name}: EVM sonucu doğrulanmadı; exact allowance sonrası son simülasyon zorunlu.`);
            return {
                success: false,
