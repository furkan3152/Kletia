...
const PROTOCOL_META: Record<string, { name: string; symbol: string; tokenAddress: `0x${string}` }> = {
    ...
    weETH:  { name: "ether.fi (weETH)",       symbol: "weETH",  tokenAddress: LSD_TOKENS.weETH  },
    ezETH:  { name: "Renzo (ezETH)",          symbol: "ezETH",  tokenAddress: LSD_TOKENS.ezETH  },
    wrsETH: { name: "Kelp (wrsETH)",          symbol: "wrsETH", tokenAddress: LSD_TOKENS.wrsETH },
};
...
export async function getLiquidStakingPositions(userAddress: string) {
    ...
    for (const [key, meta] of Object.entries(PROTOCOL_META)) {
        try {
            ...
            console.warn(`⚠️ ${meta.symbol} bakiyesi okunamadı:`, {
                name:
