...
async function eventLogs(
  ...
  while (cursor <= toBlock) {
    const end = cursor + chunkSize - 1n > toBlock
      ? toBlock
      : cursor + chunkSize - 1n;
    try {
...
async function collectHistoricalUsersFromArcScan(
  ...
  if (!response.ok) {
    throw new Error(`ArcScan historical log request failed with HTTP ${response.status}.`);
  }
  ...
  if (!Array.isArray(payload.result)) {
    throw new Error(`ArcScan historical log response is invalid: ${payload.message ?? 'unknown'}.`);
  }
  if (payload.result.length >= 1_000) {
    throw new Error('ArcScan historical log result reached its 1000-row safety limit.');
  }
  const users = new Set<Address>();
  for (const item of payload.result) {
    ...
    const itemBlock = typeof record.blockNumber === 'string'
    ...
    if (
      ...
      throw new Error('ArcScan historical event topics do not match the requested event.');
    }
...
async function collectUsersWithVerifiedTail(
  ...
  const tailFrom = latestBlock >= RPC_TAIL_BLOCKS
    ? latestBlock - RPC_TAIL_BLOCKS + 1n
    : 0n;
  const historicalTo = tailFrom > deploymentBlock
    ? tailFrom - 1n
    : deploymentBlock - 1n;
  ...
  addUsers(users, await collectUsers(
    address,
    event,
...
function accruedAmountAt(
  ...
  if (since > targetTimestamp) {
    throw new Error('Arc liability checkpoint is after the pinned target timestamp.');
  }
  return (
...
async function snapshot(
  ...
  const [
    ...
    Promise.all([...vaultUsers].map((user) =>
      arcPublicClient.readContract({
        address: ARC_CONTRACTS.Vault,
        abi: VAULT_ABI,
    ...
    Promise.all([...stakingUsers].map((user) =>
      arcPublicClient.readContract({
        address: ARC_CONTRACTS.Staking,
        abi: STAKING_ABI,
  ...
  if (mappedPrincipal !== totalDeposited) {
    throw new Error('Vault user principal does not reconcile to totalDeposited.');
  }
  ...
  const vaultCurrentLiability = vaultStates.reduce(
    (sum, value) => sum + value[0] + value[2] + accruedAmountAt(
      value[0], apyBps, value[1], currentTimestamp,
    ),
    0n,
  );
  const vaultTarget = vaultStates.reduce(
    (sum, value) => sum + value[0] + value[2] + accruedAmountAt(
      value[0], apyBps, value[1], targetTimestamp,
    ),
    0n,
  );
  const stakingCurrentLiability = stakingStates.reduce(
    (sum, value) => sum + value[2] + accruedAmountAt(
      value[0], aprBps, value[1], currentTimestamp,
    ),
    0n,
  );
  const stakingTarget = stakingStates.reduce(
    (sum, value) => sum + value[2] + accruedAmountAt(
      value[0], aprBps, value[1], targetTimestamp,
    ),
    0n,
  );
  ...
  return {
    ...
    vaultTarget,
    vaultDelta: vaultTarget > vaultBalance
    ...
    stakingTarget,
    stakingDelta: stakingTarget > rewardPoolBalance
...
async function main(): Promise<void> {
  ...
  if (chainId !== NETWORKS.arc.chainId) {
    throw new Error(`Arc chain mismatch: expected 5042002, received ${chainId}.`);
  }
  if (!vaultCode || vaultCode === '0x' || !stakingCode || stakingCode === '0x') {
  ...
  if (
    ...
    throw new Error('ARC_PRIVATE_KEY signer is not the live Vault and Staking owner.');
  }
  ...
  const verificationBlock = await arcPublicClient.getBlockNumber({ cacheTime: 0 });
  ...
  if (!vaultCovered || !stakingCovered) {
    throw new Error('Arc reserve post-state does not cover a full 30-day horizon.');
  }
