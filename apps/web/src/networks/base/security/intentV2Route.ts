...
const uniqueCanonicalAddresses = (addresses: readonly Address[]): Address[] => {
...
const hasExactPolicyTargets = (
  ...
  return (
    ...
    actual.every(
      (target, index) =>
        isCanonicalAddress(target) && sameAddress(target, expected[index]),
    )
  );
};
...
const hasValidV2ConfigEvidence = (
  ...
  (evidence.schemaVersion === "kletia_base_intent_v2_deployment_v2" ||
    (evidence.schemaVersion === "kletia_base_intent_v2_deployment_v1" &&
      adapterKind === "uniswap_v2_compatible")) &&
  evidence.adapterKind === adapterKind &&
  (parseUnsignedInteger(evidence.observedAtBlock) ?? 0n) > 0n &&
...
const normalizedIntentToken = (
  ...
  sameAddress(token, NATIVE_TOKEN_SENTINEL) ? wrappedNative : token;
...
const hasCanonicalUniV2AdapterData = (
  ...
  try {
    ...
    if (
      new Set(path.map((token) => token.toLowerCase())).size !== path.length ||
      !sameAddress(path[0], normalizedIntentToken(tokenIn, wrappedNative)) ||
      !sameAddress(
        path[path.length - 1],
...
const hasCanonicalUniV3AdapterData = (
  ...
  try {
    ...
    if (byteLength < 43 || byteLength > 112 || (byteLength - 20) % 23 !== 0) {
      return false;
    }
    ...
    const readAddress = (offset: number): Address =>
      getAddress(`0x${raw.slice(offset * 2, (offset + 20) * 2)}`);
    ...
    for (let hop = 0; hop < hopCount; hop += 1) {
      const fee = Number.parseInt(raw.slice(cursor * 2, (cursor + 3) * 2), 16);
    ...
    return (
      ...
      sameAddress(tokens[0], normalizedIntentToken(tokenIn, wrappedNative)) &&
      sameAddress(
...
const hasCanonicalAdapterData = (
  ...
  if (route.adapterKind === "uniswap_v3_swaprouter02") {
    return (
      route.adapterDataEncoding === "uniswap_v3_packed_path_v1" &&
...
const hasFailClosedV2Coverage = (
  ...
  return (
    ...
    coverage.observedAtBlock === route.configEvidence.observedAtBlock &&
...
const hasValidV2Economics = (
  ...
  const quotedGross = parseUnsignedInteger(economics.quotedGrossAmountOut);
  ...
  const maximumFee = parseUnsignedInteger(economics.maximumFeeAtSignedCap);
  const netMinimum = parseUnsignedInteger(economics.netMinimumAmountOut);
  const userMinimum =
  ...
  const expectedGrossMinimum =
    (quotedGross * (10_000n - BigInt(economics.slippageBps))) / 10_000n;
  const expectedEstimatedFee =
    (grossMinimum * BigInt(economics.observedFeeBps)) / 10_000n;
  const expectedMaximumFee =
    (grossMinimum * BigInt(economics.maxFeeBps)) / 10_000n;
  const quoteDerivedNetMinimum = grossMinimum - expectedMaximumFee;
  const expectedNetMinimum =
    checkedUserMinimum !== null && checkedUserMinimum > quoteDerivedNetMinimum
      ? checkedUserMinimum
...
export const isBaseIntentRouterV2SwapBinding = (
  ...
  try {
    ...
    if (
      ...
      parseDecodedInteger(decodedIntent.maxFeeBps) !== BigInt(intent.maxFeeBps)
    ) {
      return false;
    }
  } catch {
