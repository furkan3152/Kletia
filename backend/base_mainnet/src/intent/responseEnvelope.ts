...
import {
  resolveConfiguredBaseSwapExecution,
} from '../config/baseIntentRouterV2Environment.js';
import {
...
} from './baseIntentRouterV2.js';
import {
  ERC20_EXACT_APPROVAL_ABI,
  KLETIA_INTENT_ROUTER_V2_ABI,
} from './baseIntentRouterV2Abis.js';
import {
...
} from '../creator/launchFactoryV2.js';
import {
...
const BASE_INTENT_V2_EXECUTION_MODE =
  'kletia_intent_router_v2' as const;
...
function assertArcExternalEntityResolutionBinding(
  ...
  if (
    ...
    assertOfficialRecipientBindings(
      entityResolution,
      rawResult,
      action,
    );
...
function assertOfficialRecipientBindings(
  ...
  if (action === 'official_memo_send') {
    ...
    try {
      ...
      if (!sameAddress(getAddress(String(target)), ARC_OFFICIAL_ADDRESSES.USDC)) {
...
function assertGenericEntityResolutionBinding(
  ...
  if (
    ...
    const approvalMatches = allRawApprovals.some((approval) =>
      isRecord(approval) &&
...
function checkedLaunchV2Evidence(
  ...
  if (
    ...
    value.supplyPolicy !==
      'fixed_full_supply_to_recipient' ||
  ...
  const predictedAddress = checkedLaunchAddress(
    value.predictedAddress,
  );
  const ownerTimelock = checkedLaunchAddress(
    value.ownerTimelock,
  );
  const treasurySafe = checkedLaunchAddress(value.treasurySafe);
  const pendingTreasury = checkedLaunchAddressAllowZero(
    value.pendingTreasury,
  );
  const userSalt = checkedLaunchBytes32(value.userSalt);
  const factoryCodehash = checkedLaunchBytes32(
    value.factoryCodehash,
  );
  const totalSupply = checkedLaunchDecimal(
    value.totalSupply,
    false,
  );
  const maxDeploymentFee = checkedLaunchDecimal(
    value.maxDeploymentFee,
  );
  const deploymentFee = checkedLaunchDecimal(
    value.deploymentFee,
  );
  const nativeValue = checkedLaunchDecimal(value.value);
  const factoryFeeCap = checkedLaunchDecimal(
    value.factoryFeeCap,
    false,
  );
  const observedAtBlock = checkedLaunchDecimal(
...
function checkedV2PolicyTargets(
  ...
  const normalized = value.map((target) =>
    checkedV2Address(target),
  );
  const suppliedKeys = normalized.map((target) =>
    target.toLowerCase(),
  );
  ...
  const expectedKeys = new Set(
    expected.map((target) => target.toLowerCase()),
  );
  if (
    ...
    suppliedKeys.some(
      (key, index) =>
        ...
        key !== expected[index].toLowerCase(),
    )
  ) {
    return v2ResponseError();
  }
...
function checkedV2ConfigEvidence(
  ...
  const routerCodehash = checkedV2Hex(
    value.routerCodehash,
    true,
  );
  const wrappedNativeCodehash = checkedV2Hex(
    value.wrappedNativeCodehash,
    true,
  );
  const adapterConfigHash = checkedV2Hex(
    value.adapterConfigHash,
    true,
  );
...
function checkedV2Intent(
  ...
  if (
    ...
    deadline.value - issuedAt.value >
      MAX_BASE_INTENT_V2_TTL_SECONDS ||
...
function checkedV2Economics(
  ...
  const quotedGross = checkedV2Decimal(
    value.quotedGrossAmountOut,
    false,
  );
  const grossMinimum = checkedV2Decimal(
    value.grossMinimumAfterSlippage,
    false,
  );
  const estimatedFee = checkedV2Decimal(
    value.estimatedFeeAtObservedRate,
  );
  const maximumFee = checkedV2Decimal(
    value.maximumFeeAtSignedCap,
  );
  const netMinimum = checkedV2Decimal(
    value.netMinimumAmountOut,
    false,
  );
  const userMinimum =
  ...
  const expectedGrossMinimum =
    quotedGross.value * (10_000n - BigInt(slippageBps)) /
    10_000n;
  const expectedEstimatedFee =
    grossMinimum.value * BigInt(observedFeeBps) / 10_000n;
  const expectedMaximumFee =
    grossMinimum.value * BigInt(maxFeeBps) / 10_000n;
  const quoteDerivedNetMinimum =
    grossMinimum.value - expectedMaximumFee;
  const expectedNetMinimum =
    userMinimum !== null &&
    userMinimum.value > quoteDerivedNetMinimum
      ? userMinimum.value
...
function requireDecodedV2Call(
  ...
  > = [
    ...
    [
      decodedIntent.adapterConfigHash,
...
function requireBoundV2AdapterData(
  ...
  const normalizedInput = sameAddress(
    ...
    : intent.tokenIn;
  const normalizedOutput = sameAddress(
    intent.tokenOut,
    NATIVE_TOKEN_SENTINEL,
  )
    ? deployment.wrappedNative
  ...
  try {
    ...
    path = decoded[0].map((token) =>
      checkedV2Address(token),
    );
  } catch {
    return v2ResponseError();
  }
...
function normalizeV2Route(
  ...
  if (
    route.requestId !== undefined &&
    route.requestId !== context.requestId
  ) {
    return v2ResponseError();
  }
  ...
  const targetContract = checkedV2Address(
  ...
  const identities = findBoundV2Adapter(
    route,
  ...
  if (
    value !==
      (nativeInput ? checkedIntent.intent.amountIn : '0')
  ) {
    return v2ResponseError();
  }
  ...
  const economics = checkedV2Economics(
    route.economics,
    checkedIntent.intent,
  );
  ...
  const expectedOutput =
    ...
    route.expectedOutput.trim().length <= 500
      ...
      : v2ResponseError();
  const policyTargets = checkedV2PolicyTargets(
    route.policyTargets,
    [
  ...
  const deadlineMilliseconds =
    checkedIntent.numeric.deadline * 1_000n;
  if (
  ...
  if (
    [
      ...
      sameAddress(checkedIntent.intent.recipient, target),
    )
  ) {
    return v2ResponseError();
  }
...
function compareV2Routes(
  left: CheckedV2Route,
  right: CheckedV2Route,
): number {
  if (left.simulationStatus !== right.simulationStatus) {
    return left.simulationStatus === 'passed' ? -1 : 1;
  }
  const leftMinimum = BigInt(
    left.economics.netMinimumAmountOut,
  );
  const rightMinimum = BigInt(
    right.economics.netMinimumAmountOut,
  );
  if (leftMinimum !== rightMinimum) {
    return leftMinimum > rightMinimum ? -1 : 1;
  }
  const protocolOrder = left.protocolId.localeCompare(
    right.protocolId,
  );
  if (protocolOrder !== 0) return protocolOrder;
  const adapterKindOrder = left.adapter.kind.localeCompare(
    right.adapter.kind,
  );
...
function requireSharedV2Intent(
...
function checkedV2Coverage(
  ...
  const compiledRouteCount = checkedV2Count(
    value.compiledRouteCount,
  );
  const simulatedRouteCount = checkedV2Count(
    value.simulatedRouteCount,
  );
  const eligibleRouteCount = checkedV2Count(
    value.eligibleRouteCount,
  );
  const unsupportedQuoteCount = checkedV2Count(
    value.unsupportedQuoteCount,
  );
  const observedAtBlock = checkedV2Decimal(
    value.observedAtBlock,
    false,
  ...
  return {
    ...
    sharedExclusiveNonce,
    rankingMetric:
      'simulation_then_guaranteed_net_minimum' as const,
    noLegacyFallback: true as const,
  };
}

function checkedV2Ranking(
  value: unknown,
  routes: readonly CheckedV2Route[],
) {
  ...
  const simulationPassedCount = checkedV2Count(
    value.simulationPassedCount,
  );
  ...
  if (
    ...
    eligibleRouteCount !== routes.length ||
    simulationPassedCount !==
      routes.filter(({ simulationStatus }) =>
        simulationStatus === 'passed',
      ).length ||
    deferredUntilApprovalCount !==
      routes.filter(({ simulationStatus }) =>
  ...
  const rankedRoutes = value.rankedRoutes.map((ranked, index) => {
    ...
    if (
      ...
      ranked.quotedGrossAmountAtomic !==
        route.economics.quotedGrossAmountOut ||
    ...
    return {
      ...
      name: route.name,
      guaranteedNetMinimumAtomic:
        route.economics.netMinimumAmountOut,
      quotedGrossAmountAtomic:
        route.economics.quotedGrossAmountOut,
  ...
  return {
    ...
    stage:
      'final_routes_after_intent_v2_runtime_and_simulation' as const,
    primaryMetric:
      'guaranteed_net_minimum_amount_out' as const,
...
function checkedV2QuoteCoverage(
  ...
  const requestedSourceCount = checkedV2Count(
    value.requestedSourceCount,
  );
  const responsiveSourceCount = checkedV2Count(
    value.responsiveSourceCount,
  );
  const sourceWithRoutesCount = checkedV2Count(
    value.sourceWithRoutesCount,
  );
  const unavailableSourceCount = checkedV2Count(
    value.unavailableSourceCount,
  );
  const totalQuotedRouteCount = checkedV2Count(
    value.totalQuotedRouteCount,
  );
  ...
  const sources = value.sources.map((source, index) => {
    ...
    const quotedRouteCount = checkedV2Count(
      source.quotedRouteCount,
    );
    const attemptedQuoteCount = checkedV2Count(
      source.attemptedQuoteCount,
    );
  ...
  if (
    ...
    [...requiredSources].some((requiredSource) =>
      !sources.some(
        (source) =>
...
function createBaseIntentV2Envelope(
  ...
  if (
    ...
    (
      rawResult.action !== undefined &&
      rawResult.action !== 'swap'
    ) ||
    !Array.isArray(rawResult.allRoutes) ||
  ...
  const coverage = checkedV2Coverage(
    rawResult.intentRouterV2Coverage,
    routes,
  );
  const rankingEvidence = checkedV2Ranking(
    rawResult.rankingEvidence,
    routes,
  );
...
function createLaunchFactoryV2Envelope(
  ...
  const value = checkedLaunchDecimal(
    decimalValue(
      rawResult.value ?? rawResult.amountInWei,
      'launchV2.value',
    ),
  );
  ...
  if (
    ...
    !sameAddress(
      checkedLaunchAddress(decodedRecipient),
      userAddress,
    ) ||
  ...
  try {
    ...
    if (rawResult.isNativeIn === true) {
      return (
        ...
        sameHex(calldata, depositCalldata)
      );
    }
    return (
...
function normalizeRoute(
  ...
  const routeValue =
    route.value ??
    (fallback.isNativeIn ? fallback.value : '0');
...
export function createIntentResultEnvelope(
  ...
  const hasLaunchV2Marker =
    hasLaunchFactoryV2Marker(rawResult);
  if (hasLaunchV2Marker) {
    ...
    assertGenericEntityResolutionBinding(
      entityResolution,
      rawResult,
      action,
    );
  ...
  if (
    network === 'base' &&
    action === 'deploy_token' &&
    process.env.BASE_TOKEN_DEPLOYMENT_MODE?.trim() ===
      'launch_v2'
  ) {
    return launchV2ResponseError();
  }
  ...
  const effectiveQuoteExpiresAt = Math.min(
    clampToRecipientResolutionExpiry(
      quoteExpiresAt,
      entityResolution,
    ),
