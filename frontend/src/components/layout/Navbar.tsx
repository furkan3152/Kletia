...
import { Bot, CreditCard, Menu, Zap } from 'lucide-react';
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="flex flex-col">
          <h1 className="text-xl md:text-3xl font-black text-[#1A1A1A] dark:text-white tracking-tighter uppercase leading-none flex items-center gap-1 md:gap-2">
            KLETIA 
            {isArcMode ? (
              <span className="text-[10px] md:text-xs font-bold tracking-normal bg-[#8B5CF6] text-white px-2 py-0.5 border-[2px] border-[#1A1A1A] dark:border-[#4B5563] shadow-[2px_2px_0_#1A1A1A] dark:shadow-[2px_2px_0_#475569] lowercase">
                built on arc
              </span>
            ) : (
              <span className="text-[10px] md:text-xs font-bold tracking-normal bg-[#0052FF] text-white px-2 py-0.5 border-[2px] border-[#1A1A1A] dark:border-[#4B5563] shadow-[2px_2px_0_#1A1A1A] dark:shadow-[2px_2px_0_#475569]">
                OMNI-ENGINE
              </span>
            )}
          </h1>
        ...
        {((isAgentMode && agentWallet) || (!isAgentMode && address)) && (
          <button
            onClick={(e) => {
              if (isArcMode) {
                ...
                handleFundClick(isAgentMode ? agentWallet! : address!, e);
              }
            }}
          ...
          >
            ...
            <span className="text-xs">
              {isArcMode ? 'FAUCET' : (isAgentMode ? 'FUND VAULT' : 'FUND WALLET')}
            </span>
          </button>
        )}
        ...
        <div className="flex gap-2 items-center">
          {isAgentMode && agentWallet ? (
            <div 
              ...
              className="flex items-center gap-2 px-3 md:px-4 py-1.5 border-[3px] border-[#1A1A1A] dark:border-[#4B5563] bg-white dark:bg-[#1A2841] shadow-[3px_3px_0_#1A1A1A] dark:shadow-[3px_3px_0_#475569] hover:-translate-y-1 hover:shadow-[5px_5px_0_#1A1A1A] active:translate-y-0 active:shadow-[1px_1px_0_#1A1A1A] transition-all duration-100 ease-out cursor-pointer"
              title="Click to copy"
            >
              <Zap className="w-4 h-4 md:w-5 md:h-5 text-[#10B981]" />
                ...
                return (
                  ...
                  >
                    {(() => {
                      if (!connected) {
                        return (
                          <button onClick={openConnectModal} type="button" className="px-4 py-1.5 bg-[#0052FF] text-white font-black uppercase tracking-wider text-xs md:text-sm border-[3px] border-[#1A1A1A] shadow-[3px_3px_0_#1A1A1A] hover:-translate-y-1 hover:shadow-[5px_5px_0_#1A1A1A] active:translate-y-0 active:shadow-[1px_1px_0_#1A1A1A] transition-all">
                            Connect Wallet
                          </button>
                        );
                      }
                      ...
                      if (chain.unsupported) {
                        return (
                          <button onClick={openChainModal} type="button" className="px-4 py-1.5 bg-[#EF4444] text-white font-black uppercase tracking-wider text-xs md:text-sm border-[3px] border-[#1A1A1A] shadow-[3px_3px_0_#1A1A1A] hover:-translate-y-1 hover:shadow-[5px_5px_0_#1A1A1A] active:translate-y-0 active:shadow-[1px_1px_0_#1A1A1A] transition-all">
                            Wrong Network
                          </button>
                        );
                      }
