...
export function BaseMcpHandoffPanel({
  open,
  appWallet,
  onClose,
}: Props) {
  ...
  const copyHandoff = async () => {
    try {
      await navigator.clipboard.writeText(
        handoffInstruction(BACKEND_URL),
      );
      setCopyState('copied');
    } catch {
      setCopyState('failed');
    ...
    >
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto border-[5px] border-[#1A1A1A] bg-[#F8FAFC] p-5 text-[#1A1A1A] shadow-[10px_10px_0_#0052FF] dark:border-[#4B5563] md:p-7">
        ...
        <div className="mt-5 flex items-start gap-2 border-[3px] border-[#1A1A1A] bg-[#86EFAC] p-3 text-xs font-bold">
          ...
          Every send, swap, sign or paid request remains subject to the
          official Base Account approval flow. Kletia does not hold an agent
          vault or private key.
        </div>
