...
export async function getLendingRoutes(action: 'lend' | 'borrow' | 'repay' | 'withdraw', tokenSymbol: string, amountStr: string, userAddress: string, requestedProtocol?: string) {
    ...
    if ((action === 'lend' || action === 'repay') && amountWei !== maxUint256) {
        if (balance < amountWei) throw new Error(`❌ INSUFFICIENT_FUNDS: Not enough ${safeSymbol} in wallet to perform this action.`);
    }
    ...
    // ==========================================
    // 3. MORPHO BLUE ROTASI
    // ==========================================
    rawRoutes.push({
        name: "Morpho Blue", amount: amountWei,
        expectedOutput: `${getExpectedText(action.toUpperCase(), amountWei)} via Morpho Blue`,
        routePath: `Morpho Blue ➝ [${action.toUpperCase()}]`,
        router: ROUTERS.MORPHO_BLUE, calldata: "0x0", // Requires specific MarketParams ABI encoding
        primaryTokenAddress: tokenAddr, 
    ...
    // ==========================================
    // 4. COMPOUND V3 ROTASI
    // ==========================================
    rawRoutes.push({
        name: "Compound V3", amount: amountWei,
        expectedOutput: `${getExpectedText(action.toUpperCase(), amountWei)} via Compound V3`,
        routePath: `Compound V3 ➝ [${action.toUpperCase()}]`,
        router: ROUTERS.COMPOUND_V3_USDC, calldata: "0x0", // Requires Comet ABI encoding
        primaryTokenAddress: tokenAddr, 
        primaryAmountInWei: amountWei.toString(),
        value: "0"
    });
