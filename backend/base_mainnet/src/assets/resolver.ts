...
export interface EntityClarification {
  readonly kind: 'asset' | 'recipient' | 'protocol';
  readonly code: string;
  readonly field?: AssetField | 'recipient' | 'protocol';
...
type ResolvableAsset =
  | AssetCatalogEntry
  | Omit<VerifiedPortfolioAsset, 'similarity' | 'exactMatch'>;
...
function assetFieldPolicy(
  ...
  if (network === 'base') {
    ...
    if (configured || action === 'remove_liquidity') {
      const allowed = action === 'remove_liquidity'
        ? ['tokenIn', 'tokenOut'] as const
        : Object.keys(configured || {}) as AssetField[];
      const required = action === 'remove_liquidity'
        ? []
        : allowed;
...
function trustScore(
  ...
  switch (tier) {
    case 'core': return 98 + provenanceAdjustment;
    case 'established': return 90 + provenanceAdjustment;
    case 'project': return 92 + provenanceAdjustment;
    case 'elevated': return 72 + provenanceAdjustment;
    case 'portfolio': return 55;
  }
}

function trustLabel(
  tier: AssetTrustTier,
): ResolvedAssetEvidence['trustLabel'] {
  switch (tier) {
    case 'core': return 'canonical';
    case 'established': return 'reviewed';
    case 'project': return 'project_contract';
    case 'elevated': return 'elevated_risk';
    case 'portfolio': return 'unlisted_verified';
  }
}
...
function protocolPolicy(action: string): ReadonlySet<string> | null {
  ...
  if (['lend', 'borrow', 'repay', 'withdraw', 'yield_compare'].includes(action)) {
...
function resolveProtocol(
  ...
  if (!protocol || protocol.trim().toLowerCase() === 'unknown') return undefined;
  if (network === 'arc') {
    const folded = foldAssetReference(protocol);
    const appKitAction = ['stable_swap', 'appkit_send', 'appkit_bridge'].includes(action);
...
function boundedMetadata(value: unknown, field: string, maxLength: number): string {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_resolve, reject) => {
        timer = setTimeout(() => reject(new EntityResolutionError(
          code,
          'Onchain token doğrulama süresi güvenli istek bütçesini aştı.',
          503,
        )), remaining);
...
async function discoverBasePortfolioAssets(
  ...
  const records =
    balanceResult && typeof balanceResult === 'object' &&
    Array.isArray((balanceResult as { tokenBalances?: unknown[] }).tokenBalances)
      ? (balanceResult as { tokenBalances: unknown[] }).tokenBalances
      : [];

  const allAddresses = [...new Set(records.flatMap((record): Address[] => {
    if (!record || typeof record !== 'object') return [];
    const raw = record as { contractAddress?: unknown; tokenBalance?: unknown };
    try {
  ...
  const addresses = allAddresses.length <= 32
    ? allAddresses
    : Array.from({ length: 32 }, (_value, index) =>
        allAddresses[
          Math.floor(index * (allAddresses.length - 1) / 31)
        ],
      );
  ...
  const verified = await mapWithConcurrency(shortlist, 4, async (hint) => {
    try {
      ...
      if (asset.balance <= 0n || (!exactMatch && similarity < 0.68)) return null;
...
function discoverBasePortfolioAssetsCached(
  ...
  const cacheKey =
    `${user.toLowerCase()}:${assetReferenceIdentityKey(reference)}`;
...
export async function resolveIntentEntities(
  ...
  const client: PublicClient = (
    dependencies.baseClient || basePublicClient
  ) as PublicClient;
  ...
  const securityCheck =
    ...
    ((address: Address) =>
      checkTokenSecurity(address, 'dynamic_execution'));
  ...
  for (const role of fieldsToResolve) {
    ...
    if (catalogMatches.length === 1) {
      ...
      const fixed = fixedFields[role] !== undefined &&
        assetReferenceIdentityKey(reference) ===
    ...
    if (network === 'base') {
      ...
      const exactCandidates = portfolioCandidates.filter(({ exactMatch }) => exactMatch);
    ...
    if (!workingIntent.recipient) {
      return {
        ...
        clarification: {
          ...
          field: 'recipient',
          question: 'Alıcı tam EVM adresi veya .base.eth adı olarak belirtilmelidir.',
          options: [],
        },
      };
    }
