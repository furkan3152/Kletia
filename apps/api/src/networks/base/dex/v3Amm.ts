...
export type V3RouteExecutionProfile =
  "legacy_direct" | "intent_router_v2_quote";
...
function tupleAmountAndGas(result: unknown): {
  ...
  if (Array.isArray(result)) {
    return {
      ...
      gasEstimate: typeof result[3] === "bigint" ? result[3] : null,
    };
  }
...
export async function getV3Routes(
  ...
  const executionProfile = options.executionProfile ?? "legacy_direct";
  ...
  const hubs = [TOKENS.WETH, TOKENS.USDC, TOKENS.USDBC, TOKENS.AERO].filter(
    (hub, index, all) =>
      hub.toLowerCase() !== tokenInAddr.toLowerCase() &&
      hub.toLowerCase() !== tokenOutAddr.toLowerCase() &&
      all.findIndex(
        (candidate) => candidate.toLowerCase() === hub.toLowerCase(),
      ) === index,
  );
  ...
  async function checkV3Router(
    ...
    for (const fee of DIRECT_FEE_TIERS) {
      tasks.push(async () => {
        ...
        const { amountOut, gasEstimate } = tupleAmountAndGas(result);
        return {
          amountOut,
          gasEstimate,
      ...
      for (const [firstFee, secondFee] of MULTI_HOP_FEE_PAIRS) {
        ...
        tasks.push(async () => {
          ...
          return {
            amountOut,
            gasEstimate,
            feeLabel: `${formatV3Fee(firstFee)} + ${formatV3Fee(secondFee)}`,
            hopCount: 2,
            path,
    ...
    const candidates = settled
      .filter(
        (result): result is PromiseFulfilledResult<V3QuoteCandidate> =>
    ...
    candidates.sort((left, right) =>
      left.amountOut === right.amountOut
        ? (left.gasEstimate ?? 2n ** 255n) < (right.gasEstimate ?? 2n ** 255n)
          ? -1
          : 1
        : left.amountOut > right.amountOut
          ? -1
    ...
    const routerAbi =
      name === "Uniswap" ? V3_ROUTER_02_ABI : PANCAKE_SMART_ROUTER_ABI;
    for (const candidate of candidates.slice(0, 3)) {
      const amountOutMin =
        (candidate.amountOut * BigInt(10_000 - slippageBps)) / 10_000n;
      ...
      routes.push({
        name:
          ...
          `(${candidate.feeLabel})`,
        protocolId: name === "Uniswap" ? "uniswap" : "pancakeswap",
        amountOut: candidate.amountOut,
        expectedOutput:
          `Get ~${formatUnits(candidate.amountOut, decimalsOut)} ` +
          tokenOutSymbol,
        routePath,
        typedAdapterKind: name === "Uniswap" ? "uniswap_v3_swaprouter02" : null,
        packedPath: candidate.path,
        tokenPath: candidate.tokenPath,
        path: candidate.tokenPath,
        router: routerAddr,
        ...(calldata ? { calldata } : {}),
        estimatedGasUnits: candidate.gasEstimate?.toString() ?? null,
        ...
        feeRouterCompatible: executionProfile === "legacy_direct",
      });
    }
  }

  await Promise.all([
    checkV3Router(ROUTERS.UNI_V3_QUOTER, ROUTERS.UNI_V3, "Uniswap"),
    checkV3Router(
